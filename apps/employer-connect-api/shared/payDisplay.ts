...
// posting) and ESTIMATED pay (Florence's local-market estimate) visually + textually
...
export interface PayDisplay {
  ...
  label: string
  /** Provenance line: "from employer posting" | "Florence estimate (local RN market)". */
  source: string
...
export function payDisplay(job: Pick<FlorenceRNJob, 'listedPayMin' | 'listedPayMax' | 'listedPayUnit' | 'estimatedPayMin' | 'estimatedPayMax' | 'estimatedPayConfidence'>): PayDisplay {
  ...
  if (job.estimatedPayMin != null) {
    ...
    return { kind: 'estimated', min: job.estimatedPayMin, max: job.estimatedPayMax, unit: 'hour', label: 'Estimated pay', source: 'Florence estimate (local RN market)', confidence: job.estimatedPayConfidence, amount, text: `Estimated pay ${amount} — Florence estimate (${job.estimatedPayConfidence ?? 'low'} confidence)` }
  }
