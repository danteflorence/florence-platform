...
function Staff({ children }: { children: ReactNode }) {
  const s = useSession()
  return s.staff ? <>{children}</> : <SignIn note="The Operations console requires a Florence Education staff role (super_admin / ops)." />
}
    ...
    <div className="min-h-full">
        ...
        <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-2.5">
          <div className="flex shrink-0 items-center gap-2.5">
            ...
            <div className="leading-tight">
              <div className="whitespace-nowrap"><span className="font-display text-[15px] font-bold tracking-[-0.01em] text-ink">Florence Education</span><span className="ml-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-florence-700">Employer Connect</span></div>
      ...
      <footer className="mx-auto max-w-7xl px-4 pb-10 pt-2 text-[11px] leading-relaxed text-slate-400">
        <LegalDisclaimer compact>
          Employer Connect shares an employer-ready packet only with the candidate's consent, and withholds national-origin / visa data pre-offer by design. Demo data; mock connectors unless live credentials are set.
        </LegalDisclaimer>
      </footer>
    </div>
  )
    ...
    <div className="mx-auto max-w-md py-16">
      <Card>
        ...
        <div className="space-y-3 px-5 py-4">
          <Button onClick={() => staffLogin()}>Sign in with Florence Education</Button>
          <p className="text-[11px] text-slate-400">Single sign-on via Florence Education Core. Employer logins are provisioned in the Core admin console (an <code className="font-mono">employer</code> role on the org whose id is your ATS employer id).</p>
        </div>
      </Card>
    </div>
  )
