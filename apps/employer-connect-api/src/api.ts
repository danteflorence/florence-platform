...
export interface WeeklyOperatingDash {
  generatedAt: string
  window: { days: number; since: string }
  statusReconciliation: Record<string, number>
  programs: { total: number; active: number; channels: Record<string, number>; waveTargets: number; lockedCandidates: number }
  submissions: { total: number; recent: number; byStatus: Record<string, number> }
  billingReady: { verifiedStarts: number; candidateIds: string[] }
}
...
export const api = {
  ...
  dashWeeklyOperating: () => get<WeeklyOperatingDash>('/api/ops/dashboards/weekly-operating'),
  ...
  createProgram: (body: { employerId: string; name: string; targetCount: number; waveStructure: number[]; channel?: Program['channel']; perRnMonthlyFeeUsd?: number; ownerNames?: string[] }) => post<ProgramOverview>('/api/ops/programs', body),
