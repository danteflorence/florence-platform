...
import {
  ...
  MetricCard as FlorenceMetricCard,
  type FlorenceTone,
  type ButtonVariant,
...
const FDS_TONE: Record<Tone, FlorenceTone> = {
  brand: 'primary',
  success: 'success',
  warn: 'warning',
  danger: 'danger',
...
export function Badge({ tone = 'neutral', children }: { tone?: Tone; children: ReactNode }) {
  return <FlorenceBadge tone={FDS_TONE[tone]} className={cx('inline-flex items-center gap-1 text-[11px] ring-1 ring-inset', TONE[tone])}>{children}</FlorenceBadge>
}
...
export function Card({ children, className }: { children: ReactNode; className?: string }) {
  return <FlorenceCard className={cx('border border-slate-200 bg-white shadow-card', className)}>{children}</FlorenceCard>
}
...
export function Button({ variant = 'primary', className, ...rest }: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'ghost' | 'soft' | 'danger' }) {
  const mapped: Record<typeof variant, ButtonVariant> = {
    primary: 'primary',
    ghost: 'ghost',
    soft: 'secondary',
    danger: 'danger',
  }
  return <FlorenceButton {...rest} variant={mapped[variant]} size="sm" className={cx(className)} />
}
...
export function StatCard({ label, value, sub, tone = 'neutral' }: { label: string; value: ReactNode; sub?: ReactNode; tone?: Tone }) {
  return <FlorenceMetricCard label={label} value={value} hint={sub} tone={FDS_TONE[tone]} className="px-5 py-4" />
}
