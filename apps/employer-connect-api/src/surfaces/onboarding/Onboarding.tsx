...
// Renders for anyone; the connect actions require a Florence Education (Core) sign-in.
...
export default function Onboarding() {
  ...
  const onConnected = (imported: number) => { setDone({ imported }); setMsg({ tone: 'ok', text: `Connected — imported ${imported} open requisition(s). Florence Education nurses can now be matched and submitted into your ATS.` }) }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 py-4">
      <div className="text-center">
        <Badge tone="brand"><Icon.sparkle className="h-3 w-3" />Florence Education Employer Connect</Badge>
        <h1 className="mt-3 font-display text-3xl font-bold tracking-tight text-ink">Add the Florence Education nurse pipeline to your ATS</h1>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <Card>
            ...
            <div className="space-y-3 px-5 py-4">
              ...
              {!authed && <p className="text-[11px] text-slate-400">You'll sign in with Florence Education, then authorize your ATS.</p>}
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="space-y-3 px-5 py-4">
              <p className="text-sm text-slate-600">Greenhouse admins: create a Candidate Ingestion API key and paste it — Florence Education becomes a sourcing partner on your jobs.</p>
      ...
      <p className="text-center text-[11px] leading-relaxed text-slate-400">Florence Education only ever shares a candidate's employer-ready packet with their explicit consent, and withholds national-origin / visa data pre-offer by design. Start &amp; retention are verified via HRIS/attestation, never bare ATS status.</p>
    </div>
  )
}
