...
// role: certifies authority, fills posting detail → Florence Education mints a displayable job
...
const CERT_TEXT = 'I am authorized to post and promote this job on behalf of my organization, and I authorize Florence Education to display, summarize, and promote this role to Florence Education nurses.'
...
export default function ClaimJob() {
  ...
  return (
    <div className="mx-auto min-h-screen max-w-2xl px-4 py-10">
      ...
      <p className="mt-8 text-center text-xs text-slate-400">Florence Education delivers licensed RN capacity on a per-RN/month basis, billed after start. Claiming creates a Florence Education role from your own authorized details — nothing is copied from any third-party site.</p>
    </div>
  )
}
...
function ClaimForm({ token, view }: { token: string; view: ClaimPrefillData }) {
  ...
  if (done) return (
    <Card className="border-emerald-200 bg-emerald-50 p-6">
      ...
      <p className="mt-1 text-sm text-emerald-700">Your role is now live in Florence Education. A Florence Education advisor will present licensed RN packets and coordinate interviews. You can review the candidate-facing card at <a className="underline" href={`/jobs/${done}`}>/jobs/{done.slice(0, 8)}…</a></p>
    </Card>
  )
  ...
  return (
    <Card className="p-6">
      ...
      <p className="mt-1 text-sm text-slate-600">Confirm your details to publish this role in Florence Education and receive licensed RN packets.</p>
