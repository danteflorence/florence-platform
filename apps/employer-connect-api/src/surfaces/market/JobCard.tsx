...
// POST body. Listed pay (from the posting) and estimated pay (Florence Education local-market
  ...
  return (
    <div className="mx-auto min-h-screen max-w-2xl px-4 py-10">
      <header className="mb-6 flex items-center gap-2">
        <span className="font-display text-xl font-bold tracking-tight text-florence-700">Florence Education</span>
      ...
      <p className="mt-8 text-center text-xs text-slate-400">
        Florence Education surfaces publicly posted RN roles and Florence Education-matched opportunities. Expressing interest is not a job
...
function InterestForm({ jobId, frnClickId, cta }: { jobId: string; frnClickId?: string; cta: PublicJobCardData['cta'] }) {
  ...
  const heading = isApply ? 'Apply with Florence Education' : 'Express interest'
  ...
  if (doneRef) return (
    <Card className="border-emerald-200 bg-emerald-50 p-6">
      ...
      <p className="mt-1 text-sm text-emerald-700">A Florence Education advisor will reach out about your fastest licensure + start path. Nothing is shared with the employer yet.</p>
  ...
  return (
    <Card className="p-6">
      ...
      <p className="mt-0.5 text-sm text-slate-500">{isApply ? 'Florence Education can submit your packet to this partner once you’re licensed and consent — this starts that path.' : 'Not an application — just lets a Florence Education advisor map your path to this role.'}</p>
      <div className="mt-4 space-y-3">
        ...
        <label className="flex items-start gap-2 text-sm text-slate-600">
          ...
          <span>I consent to Florence Education contacting me about this and similar RN opportunities. I understand nothing is shared with an employer until I’m licensed and give separate consent.</span>
        </label>
