// Program Workspace — the operator cockpit for productized RN programs
// (AMN, Kaiser, Tenet, direct-employer, or VMS-partner): overview, licensed slate (+lock wave), wave tracker, scorecard,
// effective-cost/invoice rollup, expansion gate. Ops-scoped. Employers and partners see
// only redacted licensed packets via share links, never this internal cockpit.
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-slate-900">Program Workspace</h1>
...
function CreateProgram({ onCreated }: { onCreated: (id: string) => void }) {
  ...
  const [channel, setChannel] = useState<Program['channel']>('kaiser')
  ...
  const create = async () => {
    ...
    try {
      const ov = await api.createProgram({
        ...
        waveStructure: waves.split(',').map((s) => Number(s.trim())).filter((n) => n > 0), channel,
      })
  ...
  return (
    <Card>
      ...
      <div className="grid gap-2 p-3 sm:grid-cols-6">
        ...
        <select className="rounded border border-slate-300 px-2 py-1 text-sm" value={channel} onChange={(e) => setChannel(e.target.value as Program['channel'])}>
...
function ProgramDetail({ programId, onMsg }: { programId: string; onMsg: (m: string) => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader title="Licensed slate" subtitle="Licensed + employer-ready (consent granted). Employers and partners see only redacted packets." right={<Button onClick={lockWave1} disabled={slate.eligible.length === 0}>Lock wave 1</Button>} />
        <div className="divide-y divide-slate-100">
      ...
      <Card>
        <CardHeader title="Commercial · effective cost" subtitle={`Fee ${fmt$(invoices.perRnMonthlyFeeUsd)}/RN/mo (${invoices.feeSource}). FICA offset is customer-side only — never Florence Education revenue.`} />
