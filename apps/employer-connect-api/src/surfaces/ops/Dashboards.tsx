...
export default function Dashboards() {
  ...
  const weekly = useAsync(() => api.dashWeeklyOperating(), [])
  ...
  const connectPull = async (e: { id: string; name: string; atsProvider: string; integrationStatus: string }) => {
    ...
    try {
      ...
      integration.reload(); demand.reload(); weekly.reload()
    } catch (err: any) { setConnMsg({ tone: 'err', text: String(err?.message ?? err) }) } finally { setConnBusy(null) }
  }

  if (integration.loading || demand.loading || subs.loading || ledger.loading || weekly.loading) return <div className="py-10"><Spinner label="Loading operations…" /></div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader title="Weekly operating dashboard" subtitle={`${w.window.days}-day status reconciliation, Program Workspace coverage, and billing-ready starts`} right={<Badge tone="success">{w.billingReady.verifiedStarts} billing-ready</Badge>} />
        <div className="grid gap-5 px-5 py-4 lg:grid-cols-[1.4fr_1fr]">
          <div className="space-y-2">
            {['interest', 'matched', 'packet_created', 'submitted', 'interview', 'offer', 'onboarding', 'started', 'retained'].map((stage) => (
              <div key={stage} className="flex items-center gap-3">
                <div className="w-32 shrink-0 truncate text-sm text-slate-600">{titleize(stage)}</div>
                <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-slate-100">
                  <div className={cx('h-full rounded-full', (w.statusReconciliation[stage] ?? 0) ? 'bg-florence-500' : 'bg-slate-200')} style={{ width: `${Math.min(100, Math.max(4, (w.statusReconciliation[stage] ?? 0) * 12))}%` }} />
                </div>
                <div className="w-8 shrink-0 text-right font-mono text-xs text-slate-500">{w.statusReconciliation[stage] ?? 0}</div>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <StatCard label="Active programs" value={w.programs.active} sub={`${w.programs.waveTargets} wave targets`} />
            <StatCard label="Locked candidates" value={w.programs.lockedCandidates} />
            <StatCard label="Recent submissions" value={w.submissions.recent} sub={`${w.submissions.total} total`} />
            <StatCard label="Program channels" value={Object.keys(w.programs.channels).length} sub={Object.entries(w.programs.channels).map(([k, v]) => `${titleize(k)} ${v}`).join(' · ') || 'none'} />
          </div>
        </div>
      </Card>
