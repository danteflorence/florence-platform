...
export const sapConnector: ATSConnector = {
  ...
  async submitCandidate({ packet, requisition, employer }: { packet: ApplicationPacket; requisition: JobRequisition; employer: EmployerAccount }): Promise<SubmitResult> {
    ...
    const cand = await fetch(`${BASE}/odata/v2/Candidate`, { method: 'POST', headers: { authorization: basic(), 'content-type': 'application/json' }, body: JSON.stringify({ primaryEmail: 'candidate@example.test', source: 'Florence Education' }) })
