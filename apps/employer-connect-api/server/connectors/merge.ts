...
function splitName(c?: FlorenceCandidate | null, packet?: ApplicationPacket): { first: string; last: string } {
  const full = (packet?.sharedFields['fullName'] ?? c?.fullName ?? 'Florence Candidate').trim()
...
export const mergeConnector: ATSConnector = {
  ...
  async submitCandidate({ packet, requisition, employer, candidate, resume }: { packet: ApplicationPacket; requisition: JobRequisition; employer: EmployerAccount; candidate?: FlorenceCandidate | null; resume?: ResumeFile }): Promise<SubmitResult> {
    ...
    const cand = await fetch(`${ATS_BASE}/candidates`, {
      ...
      body: JSON.stringify({ model: { first_name: first, last_name: last, ...(email ? { email_addresses: [{ value: email, email_address_type: 'PERSONAL' }] } : {}), tags: ['Florence Education'] } }),
    })
    ...
    const app = await fetch(`${ATS_BASE}/applications`, { method: 'POST', headers: headers(token), body: JSON.stringify({ model: { candidate: candidateId, job: requisition.atsRequisitionId, source: 'Florence Education' } }) })
...
export async function createMergeLinkToken(employer: { id: string; name: string }): Promise<{ linkToken: string; mode: 'live' | 'mock' }> {
  ...
  const r = await fetch(`${LINK_BASE}/create-link-token`, {
    ...
    body: JSON.stringify({ end_user_origin_id: employer.id, end_user_organization_name: employer.name, end_user_email_address: `connect+${employer.id}@florenceedu.com`, categories: ['ats'] }),
  })
