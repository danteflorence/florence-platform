...
export const ukgConnector: ATSConnector = {
  ...
  async submitCandidate({ packet, requisition, employer }: { packet: ApplicationPacket; requisition: JobRequisition; employer: EmployerAccount }): Promise<SubmitResult> {
    ...
    const t = await token()
    const r = await fetch(`${BASE}/recruiting/v1/candidates`, { method: 'POST', headers: { authorization: `Bearer ${t}`, 'content-type': 'application/json' }, body: JSON.stringify({ source: 'Florence Education', requisitionId: requisition.atsRequisitionId }) })
