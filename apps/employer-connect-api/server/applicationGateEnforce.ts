...
import { checkCoreApplicationGate, mergeCoreGateResult, type CoreGateDecision } from './coreApplicationGate'
...
export async function runApplicationGate(args: {
  ...
  const coreGate = await checkCoreApplicationGate({
    candidate,
    requisition,
    packet: gatePacket,
    action: args.action ?? 'gate_check',
    channel: args.channel,
    programId: args.programId ?? packet?.programId,
  })
  result = mergeCoreGateResult(result, coreGate)
  ...
  return { allowed, wouldBlock: !result.ok, shadow: false, overridden, result, ...(coreGate.checked ? { coreGate } : {}) }
}
