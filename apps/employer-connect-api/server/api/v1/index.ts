...
const h = (fn: (req: Request, res: Response) => unknown | Promise<unknown>) =>
  (req: Request, res: Response) => {
    Promise.resolve(fn(req, res)).catch((err) => {
      ...
      const eventId = logInternalError('[api v1 error]', err)
...
apiV1.post('/applications/:packetId/submit', requireScope('applications:submit'), h(async (req, res) => {
  ...
  const coreLock = await submitThroughCoreApplicationGate({ candidate, requisition, packet: p, action: 'submission_attempt', channel: 'ats', programId: p.programId })
...
apiV1.post('/pricing/quote', requireScope('pricing:quote'), h(async (req, res) => {
  ...
  try {
    ...
    res.json({ state, setting, role, monthlyFeePerRnUsd: d.pricing?.florence_monthly_fee_per_rn, effectiveCostPerRnMonthUsd: d.pricing?.fica_adjusted_effective_cost_per_rn_month, channel: d.pricing?.channel, note: 'FICA offset is customer effective-cost, never Florence Education revenue.' })
  } catch (e) {
