// node:sqlite backend — explicit local throwaway development only. Synchronous
// engine wrapped to satisfy the async Store contract (each method is async but
// resolves immediately). Scalar columns for filtering + a `json` TEXT column
// holding the full typed object.
