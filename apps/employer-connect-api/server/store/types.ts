// ============================================================================
// Store — the async data-access contract the whole app depends on. Postgres is
// the default backend (networked with DATABASE_URL, in-memory PGlite for local
// smoke runs). The SQLite backend is explicit local throwaway development only.
//
// It is ASYNC because Postgres clients are async (node:sqlite is sync, so its
// backend just wraps the synchronous calls when explicitly enabled).
// ============================================================================
import type {
