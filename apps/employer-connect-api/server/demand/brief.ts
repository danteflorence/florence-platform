...
export function renderBriefPdf(b: DemandBrief): Buffer {
  ...
  const items: { text: string; size?: number; bold?: boolean; gap?: number }[] = [
    ...
    { text: `Florence Education Demand Radar · ${b.route === 'amn' ? 'AMN channel' : 'direct employer'} · generated ${b.generatedAt.slice(0, 10)}`, size: 9, gap: 2 },
    ...
    { text: '2. Florence Education supply match', size: 13, bold: true, gap: 14 },
    ...
    { text: `Gross Florence Education fee: ${$(b.economics.avgGrossFeePerRnMonth)}   ·   Eligible payroll-tax offset: ${$(b.economics.avgPayrollTaxOffsetPerRnMonth)}`, gap: 3 },
    ...
    { text: 'The payroll-tax offset accrues to the employer as a tax reduction; it is not Florence Education revenue.', size: 8, gap: 2 },

    { text: '4. Compensation & benefits observed', size: 13, bold: true, gap: 14 },
    { text: `${b.compensation.listedCount} role(s) with employer-listed pay · ${b.compensation.estimatedCount} with a Florence Education local-market estimate`, gap: 3 },
    ...
    { text: 'Listed pay is from the employer posting; estimated pay is a Florence Education local-market estimate, not the employer’s posted rate.', size: 8, gap: 2 },
    ...
    { text: 'DRAFT — internal review required before sending. Planning estimates from public/partner RN demand + Florence Education supply; validate via the Workforce Economist and account review. These are Florence Education-matched opportunities, not an employer endorsement.', size: 8, gap: 14 },
  ]
