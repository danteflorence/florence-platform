...
export function eligibilityCoaching(candidate: FlorenceCandidate, job: FlorenceRNJob): EligibilityCoaching {
  ...
  if (state === 'pathway_first' && (candidate.nclexStatus === 'not_started' || candidate.nclexStatus === 'diagnostic')) whatYouNeed.push('Complete Florence Academy readiness, then register for NCLEX')
  if (candidate.employerShareConsent !== 'granted') whatYouNeed.push('Grant consent so Florence Education can prepare your packet')
  if (whatYouNeed.length === 0) whatYouNeed.push('You meet the core requirements — a Florence Education advisor will finalize your packet')
