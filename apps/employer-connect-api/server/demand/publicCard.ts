// Candidate-facing job card + public "express interest" path. Employer Connect has no
// candidate login (candidates are synced projections), so the nurse reaches a job
// through a Florence Education tracked link → this PUBLIC, redacted landing card. Compliance:
...
export function publicJobCard(job: FlorenceRNJob, state: OpportunityState = 'public'): PublicJobCard {
  ...
  const readinessNote = licState
    ? `This opportunity needs an active ${licState} RN license. Express interest and a Florence Education advisor will map your fastest licensure + start path — no application is submitted until you're licensed and you've given consent.`
    : `Express interest and a Florence Education advisor will map your fastest licensure + start path — no application is submitted until you're licensed and you've given consent.`
  return {
    id: job.id,
    title: job.title,
