...
export async function buildProposal(employerName: string, route: 'amn' | 'direct' = 'direct'): Promise<Proposal> {
  ...
  const summary = `Florence Education proposes a ${brief.pilot.recommendedFirstWaveStarts}-RN pilot for ${employerName} across ${brief.jobs.states.join(', ') || 'target states'}, focused on ${brief.pilot.topSpecialties.join(', ') || 'priority specialties'}. ${brief.supply.licensed} licensed + ${brief.supply.nearLicensed} near-licensed Florence Education nurses already map to this demand.`
  const pilotPlan = [
    ...
    route === 'amn' ? 'Routed through the AMN account team; Florence Education supplies licensed supply.' : 'Direct employer engagement with Florence Education as supply partner.',
    '30-day design sprint: packet format, employer review process, ATS/manual bridge, start attestation.',
  ]
  const termsSummary = [
    `Subscription fee: ~$${fee.toLocaleString()}/RN/month (Florence Education revenue).`,
    `Eligible employer payroll-tax (FICA) offset: ~$${brief.economics.avgPayrollTaxOffsetPerRnMonth.toLocaleString()}/RN/month — accrues to the employer, NOT Florence Education revenue.`,
...
export function renderProposalPdf(p: Proposal): Buffer {
  const items: { text: string; size?: number; bold?: boolean; gap?: number }[] = [
    ...
    { text: `Florence Education · ${p.route === 'amn' ? 'AMN channel' : 'direct'} · ${p.generatedAt.slice(0, 10)} · HUMAN REVIEW REQUIRED before sending`, size: 9, gap: 2 },
    ...
    { text: 'DRAFT — internal review required before sending. Florence Education-matched opportunities, not an employer endorsement. Estimates from public/partner demand + Florence Education supply; validate via the Workforce Economist.', size: 8, gap: 14 },
  ]
