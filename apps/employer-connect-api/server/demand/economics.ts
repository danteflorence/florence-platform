  ...
  try {
    ...
    econ = {
      ...
      assumptions: [
        ...
        'FICA offset is the customer effective-cost reducer only; Florence Education revenue = subscription fee',
    ...
    econ = {
      ...
      confidence: 'low',
      assumptions: [`pricing-api unreachable (${(e as Error).message}); standard fallback fee applied`, 'FICA offset is the customer effective-cost reducer only; Florence Education revenue = subscription fee', `state=${state || 'unknown'}`],
      createdAt: now(),
    }
  }
