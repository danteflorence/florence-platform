...
export async function buildOutreachDraft(args: { employerName: string; city?: string; state?: string; roleCategory: RoleCategory; issuedBy: string }): Promise<OutreachDraft> {
  ...
  const interestLine = aggregateInterestCount > 0
    ? `We already have ${aggregateInterestCount} Florence Education nurse(s) who have expressed interest in ${role} roles in ${m.display}.`
    : `We are building Florence Education nurse interest in ${role} roles in ${m.display}.`
  const value = `Florence Education helps healthcare employers add licensed RN capacity on a per-RN/month model, billed after a nurse starts. We present licensed RN packets and support interviews through a simple employer workflow.`
  const claimLine = claimUrl ? `\n\nClaim and promote this role in Florence Education: ${claimUrl}` : ''
  const sign = `\n\n— The Florence Education team`

  const subjects = [
    `RNs interested in your ${role} roles in ${m.display}`,
    `Licensed ${role} capacity for ${args.employerName}`,
    `${m.display}: Florence Education can supply licensed RNs`,
  ]
  ...
  const sequence: OutreachStep[] = [
    ...
    { step: 2, label: 'Follow-up 1', subject: `Re: ${subjects[0]}`, body: `Hi [First name],\n\nFollowing up — ${interestLine} If you are still hiring ${role}s in ${m.display}, we can create a role in Florence Education and start presenting licensed RN packets.${claimLine}${sign}`, mailto: mailto(`Re: ${subjects[0]}`, `Following up on licensed ${role} capacity in ${m.display}.${claimLine}`) },
    ...
    { step: 4, label: 'Close', subject: `Closing the loop — ${role} in ${m.display}`, body: `Hi [First name],\n\nI'll close this out for now. If hiring ${role}s in ${m.display} becomes a priority, Florence Education can deliver licensed RN capacity on a per-RN/month basis, billed after start. Just reply and we'll pick it back up.${sign}`, mailto: mailto(`Closing the loop — ${role} in ${m.display}`, `Reach out anytime for licensed ${role} capacity in ${m.display}.`) },
  ]
...
export function renderOutreachPdf(d: OutreachDraft): Buffer {
  const items: { text: string; size?: number; bold?: boolean; gap?: number }[] = [
    ...
    { text: `Florence Education Long-Tail Demand Radar · ${d.marketDisplay} · ${roleCategoryLabel(d.roleCategory)} · HUMAN REVIEW REQUIRED before sending`, size: 9, gap: 2 },
    { text: `Aggregate Florence Education nurse interest: ${d.aggregateInterestCount}`, size: 10, gap: 12 },
  ]
  ...
  items.push({ text: 'DRAFT — not sent. Florence Education-matched opportunity, not an employer endorsement. Personalize [First name] + sign-off and send from your own inbox.', size: 8, gap: 14 })
