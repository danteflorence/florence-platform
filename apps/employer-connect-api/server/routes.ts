...
import { databaseBackend, migrationStatus, store, uid, now, audit } from './db'
...
import { applyCoreEmployerProjection } from './packetProjection'
import { generateLicensedSlate, buildSlatePackets, lockSlate, programSubmissionChannel } from './program/slate'
import { submitThroughCoreApplicationGate } from './coreApplicationGate'
...
import { weeklyOperatingDashboard } from './statusReconciliation'
...
const h = (fn: (req: Request, res: Response) => unknown | Promise<unknown>) =>
  (req: Request, res: Response) => {
    Promise.resolve(fn(req, res)).catch((err) => {
      ...
      const eventId = logInternalError('[api error]', err)
...
const WEBHOOK_REPLAY_WINDOW_MS = Number(process.env.ATS_WEBHOOK_REPLAY_WINDOW_MS ?? 5 * 60 * 1000)

export function signAtsWebhookPayload(rawBody: string | Buffer, secret: string, timestamp = Date.now()): string {
  const body = Buffer.isBuffer(rawBody) ? rawBody : Buffer.from(rawBody)
  const signature = createHmac('sha256', secret).update(`${timestamp}.`).update(body).digest('hex')
  return `t=${timestamp},v1=${signature}`
}

export function verifyAtsWebhookSignature(args: {
  rawBody: Buffer
  ...
  if (!args.secret) return { ok: false, reason: 'webhook_secret_not_configured' }
  const header = Array.isArray(args.signatureHeader) ? args.signatureHeader[0] : args.signatureHeader
  if (!header) return { ok: false, reason: 'missing_signature' }
  const timestamp = Number(header.match(/(?:^|,)t=(\d+)(?:,|$)/)?.[1])
  const signature = header.match(/(?:^|,)v1=([a-f0-9]{64})(?:,|$)/i)?.[1]
  if (!timestamp || !signature) return { ok: false, reason: 'invalid_signature_format' }
  const nowMs = args.nowMs ?? Date.now()
  const replayWindowMs = args.replayWindowMs ?? WEBHOOK_REPLAY_WINDOW_MS
  if (Math.abs(nowMs - timestamp) > replayWindowMs) return { ok: false, reason: 'stale_signature' }
  const expected = signAtsWebhookPayload(args.rawBody, args.secret, timestamp).match(/v1=([a-f0-9]{64})/i)?.[1] ?? ''
...
function rawBodyFor(req: Request): Buffer | undefined {
...
api.get('/health', h(async (_req, res) => res.json({
  ok: true,
  at: now(),
  database: { backend: databaseBackend, migration: migrationStatus() },
...
api.post('/candidates/:id/consents/employer-share', h(async (req, res) => {
  ...
  const consentText = `I authorize Florence Education to share my employer-ready packet with ${employer.name} for ${parsed.data.purpose}. The packet may include my résumé, credential summary, Florence readiness summary, video profile, and licensure/NCLEX status and expected start-window information I approve for employer review.`
...
api.post('/ops/application-packets', h(async (req, res) => {
  ...
  try {
    const packet = await applyCoreEmployerProjection(
      buildPacket({ candidate, requisition, consent, includeDocuments: parsed.data.includeDocuments, newId: uid, nowIso: now }),
      candidate,
    )
...
api.post('/ops/application-packets/:id/submit', h(async (req, res) => {
  ...
  const gate = await runApplicationGate({ candidate, requisition, packet: p, override: overrideFromBody(req.body), auditEntity: 'packet', action: 'submission_attempt', channel: lockChannel, programId: p.programId })
  ...
  const coreLock = await submitThroughCoreApplicationGate({ candidate, requisition, packet: p, action: 'submission_attempt', channel: lockChannel, programId: p.programId })
  if (coreLock.checked && !coreLock.ok) return res.status(409).json({ error: 'Core Application Gate not cleared', status: coreLock.status ?? 'not_ready', missing: coreLock.failureCodes ?? ['core_application_gate'], reasons: coreLock.reasons ?? [coreLock.error ?? 'Core submission lock was not acquired'], subjectTo: coreLock.subjectTo, subjectToMessage: coreLock.subjectToMessage })
  if (coreLock.submissionLockId) audit('system', 'core_submission_lock_acquired', 'packet', p.id, `coreLock=${coreLock.submissionLockId}`)
...
// ── PUBLIC candidate-facing job card (reached via a Florence Education tracked link) ───
...
api.post('/public/claim/:token', h(async (req, res) => {
  ...
  try {
    const r = await authorizeAndClaim({
      ...
      certificationText: String(b.certificationText ?? 'I am authorized to post and promote this job on behalf of my organization, and I authorize Florence Education to display, summarize, and promote this role to Florence Education nurses.'),
...
api.get('/ops/dashboards/weekly-operating', h(async (req, res) => {
  const windowDays = req.query.windowDays ? Number(req.query.windowDays) : 7
  res.json(await weeklyOperatingDashboard({ windowDays }))
}))
...
api.post('/ops/programs', requireRole('ops'), h(async (req, res) => {
  ...
  audit('system', 'program_submission_channel_mapped', 'program', p.id, `${p.channel}->${programSubmissionChannel(p.channel)}`)
