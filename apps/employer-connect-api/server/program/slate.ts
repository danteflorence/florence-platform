...
import type { ApplicationPacket, FlorenceCandidate, JobRequisition, MatchResult, Program, ProgramChannel, ProgramSlate, SourceChannel } from '../../shared/types'
import type { OpportunityState } from '../../shared/opportunityState'
...
export function programSubmissionChannel(channel: ProgramChannel): SubmissionChannel {
  if (channel === 'amn') return 'amn'
  if (channel === 'vms_partner') return 'vms'
  return 'direct'
}

function programSourceChannel(channel: ProgramChannel): SourceChannel {
  if (channel === 'amn') return 'amn'
  if (channel === 'vms_partner') return 'other'
  return 'direct'
}

function programOpportunityState(channel: ProgramChannel): OpportunityState {
  if (channel === 'amn') return 'amn_channel'
  if (channel === 'vms_partner') return 'vms_channel'
...
function syntheticReq(program: Program): JobRequisition {
  ...
  return {
    ...
    sourceChannel: programSourceChannel(program.channel), importedAt: now(), lastSyncedAt: now(),
  }
}
  ...
  for (const c of candidates) {
    ...
    if (m.category === 'ready_to_submit') {
      ...
      const gate = req ? applicationGate({
        candidate: c,
        job: req,
        opportunityState: programOpportunityState(program.channel),
        opts: {
  ...
  for (const candidateId of candidateIds) {
    ...
    try {
      const packet = await applyCoreEmployerProjection(
        buildPacket({ candidate: c, requisition, consent, newId: uid, nowIso: now }),
        c,
      )
...
export async function lockSlate(programId: string, waveId: string, candidateIds: string[]): Promise<ProgramSlate> {
  ...
  const submissionChannel = programSubmissionChannel(program.channel)
  for (const candidateId of candidateIds) {
    ...
    for (const packet of packets) {
      ...
      const gate = await runApplicationGate({ candidate, requisition, packet, auditEntity: 'program', action: 'slate_lock', channel: submissionChannel, programId: program.id })
  ...
  for (const a of approvals) {
    const candidate = await store.candidates.get(a.candidateId)
    const requisition = await store.requisitions.get(a.requisitionId)
    const packet = await store.packets.get(a.packetId)
    if (!candidate || !requisition || !packet) {
      for (const l of locks) await releaseSubmissionLock(l, 'slate_lock_failed')
      throw new Error(`missing approved submission context: ${a.candidateId}`)
    }
    const coreLock = await submitThroughCoreApplicationGate({ candidate, requisition, packet, action: 'slate_lock', channel: submissionChannel, programId: program.id })
    if (coreLock.checked && !coreLock.ok) {
      for (const l of locks) await releaseSubmissionLock(l, 'slate_lock_failed')
      throw new Error(`Core submission lock blocked: ${coreLock.reasons?.[0] ?? coreLock.error ?? coreLock.status ?? a.candidateId}`)
    }
    if (coreLock.submissionLockId) audit('system', 'core_submission_lock_acquired', 'program', program.id, `candidate=${a.candidateId};coreLock=${coreLock.submissionLockId}`)
    const lock = await acquireSubmissionLock({ candidateId: a.candidateId, employerId: program.employerId, requisitionId: a.requisitionId, channel: submissionChannel })
    if (!lock.ok) {
      for (const l of locks) await releaseSubmissionLock(l, 'slate_lock_failed')
