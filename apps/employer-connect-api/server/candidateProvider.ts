// ============================================================================
// CandidateProvider — legacy local-dev sync for employer-relevant projections
// from a Pathway SQLite export into Employer Connect.
//
...
//
// Read-only: we never write to the pathway-agent database. This path is not a
// production integration; production should read Pathway through Core/Platform
// APIs once the Pathway Postgres adapter is complete.
// ============================================================================
...
const localSqliteSyncAllowed = process.env.ALLOW_SQLITE_LOCAL_DEV === '1' && process.env.NODE_ENV !== 'production'

function pathwayDbPath(): string {
  if (!localSqliteSyncAllowed) {
    throw new Error('Legacy Pathway SQLite sync is allowed only with ALLOW_SQLITE_LOCAL_DEV=1 outside production.')
  }
  if (!PATHWAY_DB) {
    throw new Error('PATHWAY_DB_PATH is required for legacy local Pathway SQLite sync.')
  }
  return PATHWAY_DB
}
...
export async function readPathwayProjections(): Promise<FlorenceCandidate[]> {
  const pathwayDb = pathwayDbPath()
  if (!existsSync(pathwayDb)) throw new Error(`pathway-agent db not found at ${pathwayDb}. Has pathway-agent been seeded/run?`)
  // Dynamic import keeps production server startup free of node:sqlite.
  // @ts-ignore - node:sqlite typings vary by @types/node version.
  const { DatabaseSync } = await import('node:sqlite')
  const db = new DatabaseSync(pathwayDb, { readOnly: true }) as any
  try {
...
/** Upsert pathway projections into Employer Connect (matched on sourceCandidateId). */
export async function syncFromPathway(): Promise<{ synced: number; inserted: number; updated: number; source: string }> {
  const source = pathwayDbPath()
  const projections = await readPathwayProjections()
  ...
  for (const proj of projections) {
    ...
    if (prior) {
      // Preserve the Employer Connect id and any consent already captured here.
  ...
  return { synced: projections.length, inserted, updated, source }
}
