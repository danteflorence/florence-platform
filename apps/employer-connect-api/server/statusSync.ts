...
export interface StatusSyncResult {
  ...
  errors: { applicationId: string; error: string; eventId?: string }[]
...
export async function runStatusSync(): Promise<StatusSyncResult> {
  ...
  for (const app of apps) {
    try {
      ...
      result.errors.push({ applicationId: app.id, error: 'internal_server_error', eventId: logInternalError('[ats-connect] status-sync application failed', e) })
    }
  }
  lastRun = result
  return result
...
export function startStatusSyncLoop(): void {
  ...
  const tick = () => {
    runStatusSync()
      ...
      .catch((e) => { logInternalError('[ats-connect] status-sync failed', e) })
  }
