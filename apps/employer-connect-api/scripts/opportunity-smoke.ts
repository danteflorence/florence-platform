...
// store; default runs on Postgres/PGlite. SQLite requires
// ATS_DB=sqlite ALLOW_SQLITE_LOCAL_DEV=1.
...
ok('P4: forecast MRR = expected starts × fee (Florence Education fee only)', fc.byMonth.every((m) => Math.abs(m.expectedMrrUsd - m.expectedStarts * 1750) <= 1))
...
ok('P4: proposal terms keep FICA customer-side (not Florence Education revenue)', prop.termsSummary.some((t) => /NOT Florence Education revenue/i.test(t)))
