...
// SDK (sdk/florencern.ts) so the SDK is verified too. Default runs on
// Postgres/PGlite; SQLite requires ATS_DB=sqlite ALLOW_SQLITE_LOCAL_DEV=1.
//
...
async function main() {
  ...
  ok('pricing: FICA framed as customer effective-cost, never Florence Education revenue', /effective-cost/i.test(quote.note) && /FICA/i.test(quote.note))
