...
Built as a module **inside Employer Connect** (reuses its job/connector/matching/
...
## Verify
`node --import tsx scripts/demand-smoke.ts` — 49 checks across P1–P9 +
account ranking, passing on the Postgres/PGlite path. Full HTTP e2e proven through the running
