...
import { getEnglishRequirement, hasPassingEnglishExam } from './english-requirements'
...
function applyEnglishGate(state: string, d: CandidateDossier, reasons: string[], blockers: string[]): number {
  const rule = getEnglishRequirement(state)
  const hasEnglish = hasPassingEnglishExam(d.englishExams)
  switch (rule.routingImpact) {
    case 'clear':
      reasons.push(`${state} English check: ${rule.label}.`)
      return 4
    case 'needs_documentation':
      reasons.push(`${state} English check: English-medium instruction/textbook documentation must be on file.`)
      blockers.push(`${state} English documentation gate: ${rule.florenceAction}`)
      return -4
    case 'requires_exam':
      if (hasEnglish) {
        reasons.push(`${state} English exam gate: passing English score is on file; verify state-specific score thresholds.`)
        return 2
      }
      blockers.push(`${state} English exam gate: ${rule.summary}`)
      return -12
    case 'route_confirm':
      if (hasEnglish) {
        reasons.push(`${state} English gate is route-dependent; English score is on file, but licensure ops must confirm CES/CP/VisaScreen route.`)
        blockers.push(`${state} route confirmation: ${rule.florenceAction}`)
        return -4
      }
      blockers.push(`${state} route-dependent English gate: ${rule.florenceAction}`)
      return -10
  }
}
...
function scoreRoute(r: RouteSpec, d: CandidateDossier): RouteOption {
  ...
  if (r.alreadyLicensed) {
    ...
    if (r.endorse) {
      ...
      const dp = ssnScore(r.destinationState)
      if (dp.blocker && !hasSsn) blockers.push(dp.blocker)
      score += applyEnglishGate(r.destinationState, d, reasons, blockers)
    } else {
  ...
  if (r.alreadyLicensed) score += applyEnglishGate(r.destinationState, d, reasons, blockers)
