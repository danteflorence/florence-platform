...
import { getEnglishRequirement, type EnglishRequirementRule } from './english-requirements'
...
export interface EndorsementState {
  ...
  englishRequirement?: EnglishRequirementRule
  highlights: string[]
...
export const ENDORSEMENT_STATES: Record<string, EndorsementState> = {
  ...
  maryland: {
    ...
    highlights: [
      ...
      'Compact state.',
      'English proficiency is route-specific for endorsement and is resolved by the central English-requirement table.',
    ],
    steps: [
    ...
    citations: [{ label: 'Maryland BON — Endorsement', note: 'Nursys verification; Live Scan fingerprints (#9300000850); $230 permanent / $70 temp. English proficiency is handled by the central route-specific rule.', url: 'https://health.maryland.gov/mbon/pages/licensure-by-endorsement.aspx' }],
  },
  wisconsin: {
...
function deriveEndorsement(target: string): EndorsementState {
  ...
  return {
    ...
    citations: [],
    englishRequirement: getEnglishRequirement(target),
  }
}
...
export function resolveEndorsement(target?: string): EndorsementState {
  ...
  const englishRequirement = getEnglishRequirement(target ?? base.state)
  const officialResources = us
    ? [boardResource(us), ...base.officialResources.filter((r) => r.url !== us.boardUrl)]
    : base.officialResources
  return { ...base, officialResources, feeUsd: fee ?? base.feeUsd, englishRequirement }
}
