...
export interface CandidateProfile {
  ...
  hasSsn?: boolean
  /** F-1 SSN application progress. Process metadata only; never the SSN itself. */
  ssnApplication?: F1SsnApplicationProgress
...
export type F1SsnApplicationStatus =
  | 'not_started'
  | 'ready_to_apply'
...
export type F1SsnWorkAuthorizationKind = 'cpt_i20' | 'on_campus' | 'ead'

export interface F1SsnAppointment {
  /** Date or date-time of the SSA/Card Center visit. Do not store SSA confirmation/reference values. */
  scheduledFor?: string
  officeName?: string
  officeCity?: string
  officeState?: string
...
/** F-1 SSN application workflow progress. Stores process metadata only. */
export interface F1SsnApplicationProgress {
  status: F1SsnApplicationStatus
  workAuthorizationKind?: F1SsnWorkAuthorizationKind
  workAuthorizationConfirmedAt?: string
  onlineApplicationStartedAt?: string
  ssaVisit?: F1SsnAppointment
  ssaVisitCompletedAt?: string
...
export type ConsularCaseStatus =
  | 'not_available'
  | 'expected_review'
  | 'expedite_eligible'
  | 'expedite_requested'
...
export interface ConsularCase {
  id: string
  candidateId: string
  status: ConsularCaseStatus
  ds160WorkflowId?: string
  visaAppointmentWorkflowId?: string
  i901OrderId?: string
  visaOutcome?: import('./constants').VisaOutcome
  consulate?: string
  appointmentDate?: string
  passportReturnedAt?: string
  arrivalGateOpenedAt?: string
  createdAt: string
  updatedAt: string
}

export type DS160WorkbenchWorkflowStatus =
  | 'draft_in_progress'
  | 'needs_candidate_data'
  | 'needs_human_qa'
  | 'qa_approved'
...
export interface DS160WorkbenchStatus {
  id: string
  candidateId: string
  workflowId: string
  status: DS160WorkbenchWorkflowStatus
  candidateAttestationRequired: true
  candidateAttestedAt?: string
  sensitiveQuestionsTotal: number
  sensitiveQuestionsAnswered: number
  ceacConfirmationCaptured: boolean
  createdAt: string
  updatedAt: string
}
...
export interface VisaAppointment {
  ...
  location?: string
  appointmentDate?: string
  mrvReceiptCaptured: boolean
  i901ReceiptQaApproved: boolean
...
export interface SevismateHandoffStatus {
  id: string
  candidateId: string
  paymentOrderId: string
  handoffId: string
  status: SevismateHandoff['status']
...
export interface CandidateDossier {
  ...
  consularCases: ConsularCase[]
  ds160WorkbenchStatuses: DS160WorkbenchStatus[]
  visaAppointments: VisaAppointment[]
  consularPaymentOrders: ConsularPaymentOrder[]
  sevismateHandoffs: SevismateHandoff[]
  sevismateHandoffStatuses: SevismateHandoffStatus[]
