...
import type {
  ...
  Confidence, ConsularPaymentStatus, ConsularPaymentOrder, F1SsnApplicationStatus,
} from './types'
import type { InterviewPrep } from './interview-prep'
import type { SsnPolicy, SsnAction } from './ssn-policy'
import type { F1SsnApplicationView } from './ssn-application'
...
export interface SsnPathwayView {
  ...
  /** F-1 SSN application workflow tracker. Process metadata only; never the SSN. */
  application: F1SsnApplicationView
...
export interface AdminMetrics {
  ...
  /** Staff-only SSN application queue for required-SSN states. Process metadata only; no SSN values. */
  ssnApplications: {
    required: number
    blockedBeforeArrival: number
    readyToApply: number
    onlineStarted: number
    visitScheduled: number
    visitCompleted: number
    cardReceived: number
    overdueOfficeVisit: number
    rows: {
      candidateId: string
      candidateName: string
      state?: string
