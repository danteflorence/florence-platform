...
```bash
cd ~/florence-work/apps/pathway-api
...
- **Database**: Production target is networked Postgres (`DATABASE_URL` + `npm run migrate`).
  Local dev defaults to in-memory PGlite for the Postgres-dialect path. Legacy SQLite is available
  only through explicit local throwaway commands such as `npm run api:local`.
