...
async function expectConsularError(label: string, fn: () => unknown | Promise<unknown>, status?: number): Promise<ConsularPaymentError | null> {
  try {
    await fn()
...
async function seedCandidate(opts: {
  ...
  sevisId?: string
  schoolCode?: string
}): Promise<{ profile: CandidateProfile; school?: SchoolProgram; sevisWorkflow?: WorkflowInstance; ds160Workflow: WorkflowInstance; visaWorkflow: WorkflowInstance }> {
  ...
  await store.identityDocuments.insert(identity)
  ...
  if (opts.withI20) {
    ...
    await store.schoolPrograms.insert(school)
  ...
  const sevisWorkflow = opts.withI20
    ...
    : undefined
  if (sevisWorkflow) await store.workflows.insert(sevisWorkflow)

  const ds160Workflow: WorkflowInstance = {
    id: `${opts.id}-ds160`,
    candidateId: opts.id,
    type: 'ds160',
    title: 'DS-160',
  ...
  await store.workflows.insert(ds160Workflow)
  const ds160Draft: FormDraft = {
    id: `${opts.id}-ds160-draft`,
    candidateId: opts.id,
    workflowId: ds160Workflow.id,
    formType: 'ds160',
    title: 'DS-160 candidate review',
    generatedAt: now(),
    sections: [{
      key: 'security',
      title: 'Security and background',
      answers: [{
        fieldId: 'prior_refusal',
  ...
  await store.formDrafts.insert(ds160Draft)
  ...
  await store.workflows.insert(visaWorkflow)
  return { profile, school, sevisWorkflow, ds160Workflow, visaWorkflow }
}
...
async function main() {
  ...
  await seedCandidate({ id: incompleteId, first: 'Noemi', last: 'Missing', withI20: false })
  const missing = await expectConsularError('order creation fails without I-20 metadata / QA-approved sevis_i20', () => createI901Order({ candidateId: incompleteId, visaType: 'F1' }), 409)
  ...
  await seedCandidate({ id: dependentId, first: 'Fiona', last: 'Dependent', visaTarget: 'F-2', withI20: false })
  const dependentOrder = await createI901Order({ candidateId: dependentId, visaType: 'F2' })
  ok('F2 dependent order is marked not required', dependentOrder.status === 'not_required' && dependentOrder.required === false)
  await expectConsularError('dependent cannot generate a SEVISmate handoff', () => createSevismateHandoff(dependentOrder.id), 409)

  const staff = mint({ role: 'ops', roles: ['ops'], email: `ops.${run}@florence.dev` })
  const candidateId = `cand-i901-ready-${run}`
  const { school, ds160Workflow, visaWorkflow } = await seedCandidate({ id: candidateId, withI20: true, qaApproved: true, sevisId: `N00${run}99`, schoolCode: 'SFR214F55555000' })
  const order = await createI901Order({
    candidateId,
    visaType: 'F1',
    payerType: 'student',
    officialFeeUsd: 350,
  ...
  await expectConsularError('candidate cannot hand off before attestation', () => createSevismateHandoff(order.id), 409)

  await attestI901Order(order.id, 'Iesha Mensah')
  const handoff = await createSevismateHandoff(order.id)
  ...
  await recordI901Receipt(order.id, {
  ...
  await expectConsularError('receipt mismatch is rejected by QA approval', () => approveI901Receipt(order.id, 'qa-smoke'), 409)
  ...
  const goodReceipt = await recordI901Receipt(order.id, {
  ...
  const approved = await approveI901Receipt(order.id, 'qa-smoke')
  ok('matching receipt is QA-approved', approved.order.status === 'receipt_qa_approved' && approved.receipt.qaStatus === 'approved')

  const blockedByDs160 = await call(`/api/workflows/${visaWorkflow.id}/appointment`, {
    method: 'POST',
    body: { consulate: 'Accra', appointmentDate: '2026-08-14' },
  })
  ok('visa appointment scheduling fails before DS-160 submission', blockedByDs160.status === 409 && /DS-160/.test(String(blockedByDs160.body?.error ?? '')))

  const unsignedDs160 = await call(`/api/workflows/${ds160Workflow.id}/record-confirmation`, {
    method: 'POST',
    body: { confirmationNumber: `AA${run}77` },
  })
  ok('DS-160 cannot be final before candidate attestation', unsignedDs160.status === 409)

  const signedDs160 = await call(`/api/workflows/${ds160Workflow.id}/review-and-sign`, {
    method: 'POST',
    body: {
      signatureName: 'Iesha Mensah',
      acknowledge: true,
      answers: [{ fieldId: 'prior_refusal', value: 'No' }],
      confirmedFieldIds: [],
    },
  })
  ok('candidate attestation signs DS-160 workbench', signedDs160.status === 200 && signedDs160.body.status === 'candidate_signed')

  const ds160Submitted = await call(`/api/workflows/${ds160Workflow.id}/record-confirmation`, {
    method: 'POST',
    body: { confirmationNumber: `AA${run}77` },
  })
  ok('candidate-submitted DS-160 confirmation is recorded after attestation', ds160Submitted.status === 200 && ds160Submitted.body.status === 'submitted')

  const afterAppt = await call(`/api/workflows/${visaWorkflow.id}/appointment`, {
    method: 'POST',
    body: { consulate: 'Accra', appointmentDate: '2026-08-14' },
  })
  ok('visa appointment scheduling succeeds after receipt QA approval and DS-160 submission', afterAppt.status === 200 && afterAppt.body.status === 'submitted' && afterAppt.body.appointmentStatus === 'interview_scheduled')

  const attended = await call(`/api/workflows/${visaWorkflow.id}/appointment-status`, {
    method: 'POST',
    token: staff,
    body: { status: 'interview_attended', occurredOn: '2026-08-14' },
  })
  ok('human staff can record interview attended status', attended.status === 200 && attended.body.status === 'interview_attended')

  const decision = await call(`/api/workflows/${visaWorkflow.id}/visa-result`, {
    method: 'POST',
    token: staff,
    body: { outcome: 'approved', decidedOn: '2026-08-14' },
  })
  ok('human staff records visa approval outcome', decision.status === 200 && decision.body.status === 'visa_approved')

  const ds160Status = (await store.ds160WorkbenchStatuses.byWorkflow(ds160Workflow.id))[0]
  const visaStatus = (await store.visaAppointments.byWorkflow(visaWorkflow.id))[0]
  const consularCase = (await store.consularCases.byCandidate(candidateId))[0]
  const handoffStatus = (await store.sevismateHandoffStatuses.byOrder(order.id))[0]
  ok('DS-160 workbench status is explicit and submitted', ds160Status?.status === 'submitted_to_ceac' && ds160Status.candidateAttestationRequired && ds160Status.ceacConfirmationCaptured)
  ok('visa appointment status writes to explicit read model', visaStatus?.status === 'visa_approved' && visaStatus.ds160Submitted && visaStatus.i901ReceiptQaApproved)
  ok('consular case status writes to Core-facing read model', consularCase?.status === 'visa_approved' && consularCase.ds160WorkflowId === ds160Workflow.id && consularCase.visaAppointmentWorkflowId === visaWorkflow.id)
  ok('SEVISmate handoff status model is tracked', handoffStatus?.status === 'sent' && handoffStatus.paymentLinkGenerated === Boolean(handoff.handoff.paymentLink))

  const eventTypes = (await store.consularPaymentEvents.byOrder(order.id)).map((e) => e.eventType)
  ...
  const milestones = (await store.ledger.byCandidate(candidateId)).map((m) => m.milestone)
  ...
  ok('Core ledger records material pathway milestones', ['SEVIS I-901 fee paid', 'DS-160 submitted', 'Visa appointment scheduled', 'Visa interview attended', 'Visa approved'].every((m) => milestones.includes(m)))

  const auditText = JSON.stringify(await store.audit.byCandidate(candidateId))
  ok('audit stream redacts PII and sensitive identifiers', !auditText.includes(school!.i20Number!) && !auditText.includes('Iesha Mensah') && !auditText.includes(`AA${run}77`) && !auditText.includes('1996-05-03'))

  const dashboard = await consularPaymentsDashboard()
  const reconciliation = await consularPaymentsReconciliation()
  ...
  ok('staff-only Pathway QA queues endpoint is available', (await call('/v1/pathway/qa/queues', { token: staff })).status === 200)
