...
// API: defaults to Postgres. With no DATABASE_URL outside production, the server
// uses in-memory PGlite so the dev loop stays Postgres-dialect without DB files.
start('api', ['--watch', '--import', 'tsx', 'server/index.ts'])
