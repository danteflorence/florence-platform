...
import { pathwayQaQueues } from '../../pathwayStatus'
...
const h = (fn: (req: Request, res: Response) => unknown | Promise<unknown>) =>
  (req: Request, res: Response) => {
    Promise.resolve(fn(req, res)).catch((err) => {
      ...
      const eventId = logInternalError('[pathway api v1 error]', err)
      res.status(500).json(internalErrorBody(eventId))
    })
  }
...
apiV1.get('/pathway/:id/status', h(async (req, res) => {
  ...
  res.json({
    ...
    consularCases: d.consularCases.map((c) => ({ id: c.id, status: c.status, ds160WorkflowId: c.ds160WorkflowId, visaAppointmentWorkflowId: c.visaAppointmentWorkflowId })),
    ds160WorkbenchStatuses: d.ds160WorkbenchStatuses.map((s) => ({
      id: s.id,
      workflowId: s.workflowId,
      status: s.status,
      candidateAttestationRequired: s.candidateAttestationRequired,
      candidateAttested: Boolean(s.candidateAttestedAt),
      sensitiveQuestionsTotal: s.sensitiveQuestionsTotal,
      sensitiveQuestionsAnswered: s.sensitiveQuestionsAnswered,
      ceacConfirmationCaptured: s.ceacConfirmationCaptured,
    })),
    visaAppointments: d.visaAppointments.map((v) => ({
      id: v.id,
      workflowId: v.workflowId,
      status: v.status,
      i901ReceiptQaApproved: v.i901ReceiptQaApproved,
      ds160Submitted: v.ds160Submitted,
      mrvReceiptCaptured: v.mrvReceiptCaptured,
      ...(v.visaOutcome ? { visaOutcome: v.visaOutcome } : {}),
    })),
  })
}))
...
apiV1.get('/pathway/qa/queues', h(async (req, res) => {
  const p = await authStaff(req, res); if (!p) return
  res.json(await pathwayQaQueues())
}))
...
apiV1.post('/consular/payments/i901/orders', h(async (req, res) => {
  ...
  const order = await createI901Order(parsed.data)
...
apiV1.patch('/consular/payments/i901/orders/:orderId', h(async (req, res) => {
  ...
  const order = await patchI901Order(req.params.orderId, body)
  res.json(await detailForOrder(order))
}))
...
apiV1.post('/consular/payments/i901/:orderId/attest', h(async (req, res) => {
  ...
  const order = await attestI901Order(req.params.orderId, parsed.data.signatureName)
  res.json(await detailForOrder(order))
}))
...
apiV1.post('/consular/payments/i901/:orderId/handoff/sevismate', h(async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() })
  res.json(await createSevismateHandoff(req.params.orderId, parsed.data.integrationMode))
}))
...
apiV1.post('/consular/payments/i901/:orderId/receipt', h(async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() })
  res.json(await recordI901Receipt(req.params.orderId, parsed.data))
}))
...
apiV1.post('/consular/payments/i901/:orderId/qa-approve', h(async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() })
  res.json(await approveI901Receipt(req.params.orderId, parsed.data.reviewer))
}))
...
apiV1.post('/consular/payments/i901/:orderId/qa-reject', h(async (req, res) => {
  ...
  const parsed = i901QaDecisionSchema.safeParse(req.body)
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() })
  res.json(await rejectI901Receipt(req.params.orderId, parsed.data.reviewer, parsed.data.notes))
}))
