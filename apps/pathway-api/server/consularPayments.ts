...
import { recordSevismateHandoffStatus, refreshConsularCase } from './pathwayStatus'
import type {
  CandidateDossier,
  CandidateProfile,
...
export async function latestI901Order(candidateId: string): Promise<ConsularPaymentOrder | null> {
  return (await store.consularPaymentOrders.byCandidate(candidateId)).find((o) => o.paymentType === 'i901_sevis') ?? null
}

export async function latestI901Receipt(orderId: string): Promise<I901Receipt | null> {
  return (await store.i901Receipts.byOrder(orderId))
    .sort((a, b) => (b.receivedAt ?? '').localeCompare(a.receivedAt ?? '') || b.id.localeCompare(a.id))[0] ?? null
}

export async function i901ReceiptApproved(candidateId: string): Promise<boolean> {
  const order = await latestI901Order(candidateId)
  if (!order) return false
  return order.status === 'receipt_qa_approved' || (await store.i901Receipts.byOrder(order.id)).some((r) => r.qaStatus === 'approved')
}

async function event(order: ConsularPaymentOrder, eventType: ConsularPaymentEvent['eventType'], metadata?: Record<string, unknown>): Promise<void> {
  await store.consularPaymentEvents.insert({ id: uid(), candidateId: order.candidateId, paymentOrderId: order.id, eventType, occurredAt: now(), ...(metadata ? { metadata } : {}) })
}
...
async function assertCandidate(candidateId: string): Promise<CandidateDossier> {
  const d = await getDossier(candidateId)
  if (!d) throw new ConsularPaymentError(404, 'candidate not found')
  return d
}

export async function evaluateI901Eligibility(candidateId: string, visaTypeArg?: ConsularVisaType): Promise<{
  eligible: boolean
  notRequired: boolean
  missing: string[]
  visaType: ConsularVisaType
  ...
  const d = await assertCandidate(candidateId)
...
export async function createI901Order(args: CreateI901OrderArgs): Promise<ConsularPaymentOrder> {
  const d = await assertCandidate(args.candidateId)
  const visaType = args.visaType ?? normalizeVisaType(d.profile.visaTarget)
  const existing = await latestI901Order(args.candidateId)
  ...
  const eligibility = await evaluateI901Eligibility(args.candidateId, visaType)
  ...
  await store.consularPaymentOrders.insert(order)
  await audit('system', 'i901_payment_order_created', 'consular_payment_order', order.id, order.candidateId, order.status)
  await event(order, 'i901_payment_order_created', { visaType: order.visaType, payerType: order.payerType })
  const consularCase = await refreshConsularCase(order.candidateId)
  emitForCandidate(order.candidateId, 'pathway.consular_case_status', { stage: 'i901_order_created', status: consularCase?.status ?? 'not_available', orderId: order.id })
  return order
}

export async function patchI901Order(orderId: string, patch: Partial<ConsularPaymentOrder>): Promise<ConsularPaymentOrder> {
  const order = await store.consularPaymentOrders.get(orderId)
  ...
  await store.consularPaymentOrders.update(updated)
  await audit('system', 'i901_payment_order_updated', 'consular_payment_order', updated.id, updated.candidateId, updated.status)
  const consularCase = await refreshConsularCase(updated.candidateId)
  emitForCandidate(updated.candidateId, 'pathway.consular_case_status', { stage: 'i901_order_updated', status: consularCase?.status ?? 'not_available', orderId: updated.id })
  return updated
}

export async function attestI901Order(orderId: string, signatureName: string): Promise<ConsularPaymentOrder> {
  const order = await store.consularPaymentOrders.get(orderId)
  if (!order) throw new ConsularPaymentError(404, 'payment order not found')
  ...
  const d = await assertCandidate(order.candidateId)
  const school = d.schoolPrograms[0]
  const attestation = {
    id: uid(),
  ...
  await store.attestations.insert(attestation)
  order.candidateAttestationId = attestation.id
  order.status = 'ready_for_sevismate'
  order.updatedAt = now()
  await store.consularPaymentOrders.update(order)
  await audit('candidate', 'i901_candidate_attested', 'consular_payment_order', order.id, order.candidateId, signatureName)
  await event(order, 'i901_candidate_attested', { attestationId: attestation.id })
  const consularCase = await refreshConsularCase(order.candidateId)
  emitForCandidate(order.candidateId, 'pathway.consular_case_status', { stage: 'i901_candidate_attested', status: consularCase?.status ?? 'not_available', orderId: order.id })
  return order
}

export async function createSevismateHandoff(orderId: string, integrationMode: SevismateHandoff['integrationMode'] = 'deep_link'): Promise<{ handoff: SevismateHandoff; order: ConsularPaymentOrder; instructions: string[] }> {
  const order = await store.consularPaymentOrders.get(orderId)
  if (!order) throw new ConsularPaymentError(404, 'payment order not found')
  if (!order.required) throw new ConsularPaymentError(409, 'I-901 payment is not required for this order')
  ...
  await store.sevismateHandoffs.insert(handoff)
  order.status = paymentLink ? 'payment_link_generated' : 'ready_for_sevismate'
  order.updatedAt = now()
  await store.consularPaymentOrders.update(order)
  await recordSevismateHandoffStatus(handoff)
  await audit('system', 'i901_handoff_sent', 'consular_payment_order', order.id, order.candidateId, integrationMode)
  await event(order, 'i901_handoff_sent', { integrationMode, fieldsSent: handoff.fieldsSent, documentsSent: handoff.documentsSent })
  const consularCase = await refreshConsularCase(order.candidateId)
  emitForCandidate(order.candidateId, 'pathway.consular_case_status', { stage: 'sevismate_handoff_sent', status: consularCase?.status ?? 'not_available', orderId: order.id })
  return {
    handoff,
    order,
    instructions: [
...
export async function recordI901Receipt(orderId: string, input: {
  filename: string
  sevisId: string
  legalName?: string
  schoolCode?: string
...
}): Promise<{ order: ConsularPaymentOrder; receipt: I901Receipt; document: PathwayDocument }> {
  const order = await store.consularPaymentOrders.get(orderId)
  if (!order) throw new ConsularPaymentError(404, 'payment order not found')
  if (!order.required) throw new ConsularPaymentError(409, 'I-901 payment is not required for this order')
  ...
  await store.documents.insert(document)
  ...
  await store.i901Receipts.insert(receipt)
  order.status = 'receipt_received'
  order.updatedAt = now()
  await store.consularPaymentOrders.update(order)
  await audit('candidate', 'i901_receipt_received', 'consular_payment_order', order.id, order.candidateId, document.filename)
  await event(order, 'i901_receipt_received', { receiptId: receipt.id, documentId: document.id })
  await pushMilestone(order.candidateId, order.consularCaseId, 'I-901 receipt uploaded')
  emitForCandidate(order.candidateId, 'consular.i901_receipt_uploaded', { orderId: order.id, receiptId: receipt.id })
  const consularCase = await refreshConsularCase(order.candidateId)
  emitForCandidate(order.candidateId, 'pathway.consular_case_status', { stage: 'i901_receipt_uploaded', status: consularCase?.status ?? 'expected_review', orderId: order.id })
  return { order, receipt, document }
}

export async function receiptMismatches(order: ConsularPaymentOrder, receipt: I901Receipt): Promise<string[]> {
  const d = await assertCandidate(order.candidateId)
  const school = d.schoolPrograms[0]
...
export async function approveI901Receipt(orderId: string, reviewer: string): Promise<{ order: ConsularPaymentOrder; receipt: I901Receipt }> {
  const order = await store.consularPaymentOrders.get(orderId)
  if (!order) throw new ConsularPaymentError(404, 'payment order not found')
  const receipt = await latestI901Receipt(order.id)
  if (!receipt) throw new ConsularPaymentError(409, 'upload the I-901 receipt before QA approval')
  const mismatches = await receiptMismatches(order, receipt)
  ...
  await store.i901Receipts.update(receipt)
  order.status = 'receipt_qa_approved'
  order.updatedAt = now()
  await store.consularPaymentOrders.update(order)

  const w = await store.workflows.get(order.consularCaseId)
  if (w && w.type === 'sevis_i20' && w.status !== 'completed') {
    applyStatus(w, 'completed')
    await store.workflows.update(w)
  }

  await audit('qa', 'i901_receipt_qa_approved', 'consular_payment_order', order.id, order.candidateId, reviewer)
  await event(order, 'i901_receipt_qa_approved', { receiptId: receipt.id, reviewer })
  await pushMilestone(order.candidateId, order.consularCaseId, 'SEVIS I-901 fee paid')
  await pushMilestone(order.candidateId, order.consularCaseId, 'I-901 receipt QA approved')
  ...
  const consularCase = await refreshConsularCase(order.candidateId)
  emitForCandidate(order.candidateId, 'pathway.consular_case_status', { stage: 'i901_receipt_qa_approved', status: consularCase?.status ?? 'expedite_eligible', orderId: order.id })
  return { order, receipt }
}

export async function rejectI901Receipt(orderId: string, reviewer: string, notes?: string): Promise<{ order: ConsularPaymentOrder; receipt: I901Receipt }> {
  const order = await store.consularPaymentOrders.get(orderId)
  if (!order) throw new ConsularPaymentError(404, 'payment order not found')
  const receipt = await latestI901Receipt(order.id)
  ...
  receipt.qaReviewerId = reviewer
  receipt.qaReviewedAt = now()
  receipt.rejectionReason = notes ?? ((await receiptMismatches(order, receipt)).join(', ') || 'Correction required')
  await store.i901Receipts.update(receipt)
  order.status = 'receipt_rejected_needs_correction'
  order.statusReason = receipt.rejectionReason
  order.updatedAt = now()
  await store.consularPaymentOrders.update(order)
  await audit('qa', 'i901_receipt_rejected', 'consular_payment_order', order.id, order.candidateId, reviewer)
  await event(order, 'i901_correction_required', { receiptId: receipt.id, reviewer, notes: receipt.rejectionReason })
  const consularCase = await refreshConsularCase(order.candidateId)
  emitForCandidate(order.candidateId, 'pathway.consular_case_status', { stage: 'i901_receipt_rejected', status: consularCase?.status ?? 'expected_review', orderId: order.id })
  return { order, receipt }
}

export async function detailForOrder(order: ConsularPaymentOrder) {
  return {
    order,
    handoffs: await store.sevismateHandoffs.byOrder(order.id),
    receipt: await latestI901Receipt(order.id),
    events: await store.consularPaymentEvents.byOrder(order.id),
  }
}
...
async function rowFor(order: ConsularPaymentOrder): Promise<PaymentDashboardRow> {
  const d = await assertCandidate(order.candidateId)
  const school = d.schoolPrograms[0]
  const wf = await store.workflows.get(order.consularCaseId)
...
export async function consularPaymentsDashboard(): Promise<PaymentDashboard> {
  const rows = await Promise.all((await store.consularPaymentOrders.all()).filter((o) => o.paymentType === 'i901_sevis').map(rowFor))
...
export async function consularPaymentsReconciliation(): Promise<PaymentReconciliation> {
  const orders = (await store.consularPaymentOrders.all()).filter((o) => o.paymentType === 'i901_sevis')
  const receipts = await store.i901Receipts.all()
  ...
  const i20ToReceiptDays = receipts
    .map((r) => {
      const o = orders.find((order) => order.id === r.paymentOrderId)
      if (!o) return null
...
export async function candidatePaymentSummary(candidateId: string) {
  const d = await assertCandidate(candidateId)
  const order = await latestI901Order(candidateId)
  const eligibility = await evaluateI901Eligibility(candidateId, order?.visaType ?? normalizeVisaType(d.profile.visaTarget))
  const receipt = order ? await latestI901Receipt(order.id) : null
  const handoff = order ? (await store.sevismateHandoffs.byOrder(order.id))[0] : null
  const school = d.schoolPrograms[0]
  return {
    i901: {
