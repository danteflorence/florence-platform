...
import { getEnglishRequirement, type EnglishRequirementRule } from '../../shared/english-requirements'
...
function englishRequirementAnswer(rule: EnglishRequirementRule, d: CandidateDossier): FormAnswer {
  const passed = d.englishExams.find((x) => x.passed)
  const accepted = rule.acceptedScores.length
    ? ` Accepted scores: ${rule.acceptedScores.map((x) => `${x.exam} ${x.score}`).join('; ')}.`
    : ''
  const note = `${rule.summary} Florence action: ${rule.florenceAction}${accepted}`

  if (rule.routingImpact === 'clear') {
    return ans(
      'english_requirement',
      'English proficiency route',
      rule.label,
      ev('derived', 'English requirement rules'),
      'medium',
      { source: rule.source, note },
    )
  }

  if (rule.routingImpact === 'needs_documentation') {
    return ans(
      'english_instruction_documentation',
      'English-instruction documentation',
      null,
      [],
      'unknown',
      { source: rule.source, note },
  ...
  return ans(
    rule.routingImpact === 'route_confirm' ? 'english_route_confirmation' : 'english_score',
    rule.routingImpact === 'route_confirm' ? 'English route confirmation' : 'English proficiency evidence',
    passed ? `${passed.exam} score on file` : null,
    passed ? ev('english_score', 'Score report') : [],
    passed ? 'medium' : 'unknown',
    { source: rule.source, note },
  )
}
...
const endorsement: Builder = (d) => {
  ...
  const answers: FormAnswer[] = [
    ...
    englishRequirementAnswer(target.englishRequirement ?? getEnglishRequirement(target.state), d),
  ]
...
const cgfns: Builder = (d) => {
  const target = d.profile.employmentState ?? d.profile.nclexState
  const englishRule = getEnglishRequirement(target)
  return [
    { key: 'identity', title: 'Applicant Identity', answers: identityAnswers(d) },
    {
      ...
      answers: [
        ...
        englishRequirementAnswer(englishRule, d),
      ],
    },
  ]
}
