...
import { buildF1SsnApplicationView } from '../shared/ssn-application'
...
export async function assembleCandidateView(candidateId: string): Promise<CandidateView | null> {
  ...
  const deficiencies = await Promise.all((await store.deficiencies.all()).filter((x) => x.candidateId === candidateId && !x.resolvedAt).map(async (x) => {
    ...
    const workflow = await store.workflows.get(x.workflowId)
    return {
      id: x.id, workflowId: x.workflowId,
      workflowShort: WORKFLOW_META[workflow?.type ?? 'ds160'].short,
  ...
  const ssnApplication = buildF1SsnApplicationView(d.profile, ssnAct, ssnPolicy)
  const ssn = {
  ...
  return {
    ...
    ledger: await store.ledger.byCandidate(candidateId),
    resources,
    help: LEGAL_HELP,
...
export async function assembleQaQueue(): Promise<QaQueueItem[]> {
  return Promise.all((await store.qaReviews.pending()).map(async (review) => {
    const [w, c] = await Promise.all([
      store.workflows.get(review.workflowId),
      store.candidates.get(review.candidateId),
    ])
...
export async function assembleQaDetail(reviewId: string): Promise<QaDetail | null> {
  const review = await store.qaReviews.get(reviewId)
  if (!review) return null
  const w = await store.workflows.get(review.workflowId)
  if (!w) return null
  const d = await getDossier(review.candidateId)
  const draft = review.formDraftId ? await store.formDrafts.get(review.formDraftId) : await store.formDrafts.byWorkflow(w.id)
  const comp = d && draft ? complianceCheck(w.type, draft, d, review.flags) : null
  const c = await store.candidates.get(review.candidateId)
  const auditEntries = await store.audit.byCandidate(review.candidateId)
  return {
    review,
    draft: draft ?? null,
    workflow: w,
...
export async function pipelineLagForCandidate(candidateId: string): Promise<PipelineLag> {
  const edu = await store.education.byCandidate(candidateId)
  const grad = edu.map((e) => e.graduationDate).filter(Boolean).sort()[0]
  return pipelineLag(grad)
}

export async function assembleAdminMetrics(): Promise<AdminMetrics> {
  ...
  const ssnRows: AdminMetrics['ssnApplications']['rows'] = []
  const ssnSummary = {
    required: 0,
    blockedBeforeArrival: 0,
    readyToApply: 0,
    onlineStarted: 0,
    visitScheduled: 0,
    visitCompleted: 0,
    cardReceived: 0,
    overdueOfficeVisit: 0,
  }
  ...
  for (const c of candidates) {
    ...
    const ssnPolicy = getSsnPolicy(c.employmentState)
    if (ssnPolicy.requirement === 'required') {
      const ssnAct = ssnAction(ssnPolicy, !!c.hasSsn)
      const ssnApp = buildF1SsnApplicationView(c, ssnAct, ssnPolicy)
      const status = c.hasSsn ? 'card_received' : ssnApp.status
      const overdue = !!ssnApp.officeVisitDueDate && ssnApp.officeVisitDueDate < todayIso && !c.ssnApplication?.ssaVisitCompletedAt && !c.hasSsn
      ssnSummary.required++
      if ((c.arrivalStatus ?? 'abroad') !== 'arrived' && !c.hasSsn) ssnSummary.blockedBeforeArrival++
      if (status === 'ready_to_apply' || (!c.hasSsn && ssnApp.nextStep?.key === 'confirm_work_authorization')) ssnSummary.readyToApply++
      if (status === 'online_application_started') ssnSummary.onlineStarted++
      if (status === 'ssa_visit_scheduled') ssnSummary.visitScheduled++
      if (status === 'ssa_visit_completed') ssnSummary.visitCompleted++
      if (status === 'card_received') ssnSummary.cardReceived++
      if (overdue) ssnSummary.overdueOfficeVisit++
      ssnRows.push({
        candidateId: c.id,
        candidateName: `${c.legalFirstName} ${c.legalLastName}`,
        state: c.employmentState,
        status,
        nextAction: c.hasSsn ? 'SSN card received - license gate can proceed' : ssnApp.nextStep?.label ?? 'Confirm pathway status',
        dueDate: ssnApp.officeVisitDueDate,
        risk: overdue ? 'red' : (c.arrivalStatus ?? 'abroad') !== 'arrived' ? 'neutral' : status === 'ssa_visit_completed' ? 'orange' : status === 'card_received' ? 'green' : 'orange',
      })
    }
  }
  ...
  const payments = await consularPaymentsDashboard()
  const reconciliation = await consularPaymentsReconciliation()

  const ledgerAll = await store.ledger.all()
  ...
  return {
    ...
    byStatus,
    byType,
    pendingQa: (await store.qaReviews.pending()).length,
    ...
    ssnApplications: {
      ...ssnSummary,
      rows: ssnRows.sort((a, b) => {
        const riskRank = { red: 3, orange: 2, neutral: 1, green: 0 }
        return (riskRank[b.risk] - riskRank[a.risk]) || (a.dueDate ?? '9999-12-31').localeCompare(b.dueDate ?? '9999-12-31')
      }),
    },
  }
}

export async function candidateSummaries(): Promise<CandidateSummary[]> {
  return Promise.all((await store.candidates.all()).map(async (c) => {
    const d = await getDossier(c.id)
    if (!d) throw new Error(`candidate dossier missing for ${c.id}`)
