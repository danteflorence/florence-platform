...
export async function seedIfEmpty(): Promise<void> {
  if ((await store.candidates.count()) > 0) return
  for (const s of SEEDS) {
    ...
    for (const o of s.identity) await store.identityDocuments.insert(link(o) as any)
    for (const o of s.education) await store.education.insert(link(o) as any)
    for (const o of s.employment) await store.employment.insert(link(o) as any)
    for (const o of s.licenses) await store.licenses.insert(link(o) as any)
    for (const o of s.visaHistory) await store.visaHistory.insert(link(o) as any)
    for (const o of s.travelHistory) await store.travelHistory.insert(link(o) as any)
    for (const o of s.schoolPrograms) await store.schoolPrograms.insert(link(o) as any)
    for (const o of s.employerOffers) await store.employerOffers.insert(link(o) as any)
    for (const o of s.financing) await store.financing.insert(link(o) as any)
    for (const o of s.englishExams) await store.englishExams.insert(link(o) as any)
    for (const o of s.nclex) await store.nclex.insert(link(o) as any)
    for (const o of s.schoolPrograms) {
      await store.documents.insert({
    ...
    const sevis = (await store.workflows.byCandidate(id)).find((w) => w.type === 'sevis_i20')
    if (sevis && s.schoolPrograms.length > 0) {
      applyStatus(sevis, 'qa_approved'); await store.workflows.update(sevis)
      ...
      if (s.profile.legalFirstName === 'Priya') {
        await attestI901Order(order.id, 'Priya Nair')
        await createSevismateHandoff(order.id)
      }
      if (s.profile.legalFirstName === 'Grace') {
        await attestI901Order(order.id, 'Grace Wanjiru')
        await createSevismateHandoff(order.id)
        ...
        await approveI901Receipt(order.id, 'Seed QA')
      }
    }
  }

  // Demo: a board deficiency on the New York application — shows the guided response flow.
  const ny = (await store.workflows.all()).find((w) => w.type === 'newyork_rn_exam')
  if (ny) {
    ...
    const w = await store.workflows.get(ny.id)
    if (w) {
