...
export const api = {
  ...
  uploadDocument: (candidateId: string, body: { kind: string; filename: string; imageBase64?: string; mediaType?: string; textContent?: string }) => post<{ id: string; extracted: boolean; fields?: Record<string, string>; confidence?: string; notes?: string[] }>(`/api/candidates/${candidateId}/documents`, body),
  confirmDocument: (candidateId: string, docId: string, fields: Record<string, string>) => post<{ ok: boolean }>(`/api/candidates/${candidateId}/documents/${docId}/confirm`, { fields }),
  ...
  updateSsnApplication: (candidateId: string, body: { action: string; workAuthorizationKind?: string; scheduledFor?: string; officeName?: string; officeCity?: string; officeState?: string }) =>
    post<{ hasSsn: boolean; application: CandidateView['ssn']['application']['progress'] }>(`/api/candidates/${candidateId}/ssn-application`, body),
