...
export default function AdminDashboard() {
  ...
  const ssnStatusLabel: Record<string, string> = {
    not_started: 'Not started',
    ready_to_apply: 'Ready',
    online_application_started: 'Online started',
    ssa_visit_scheduled: 'Visit scheduled',
    ssa_visit_completed: 'Visit completed',
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader
          title="F-1 SSN Application Queue"
          subtitle="Required-state candidates only. Tracks SSA process metadata; Florence never stores the number, last four, or confirmation/reference values."
          right={<Badge tone={m.ssnApplications.overdueOfficeVisit ? 'danger' : m.ssnApplications.required ? 'warn' : 'success'}>{m.ssnApplications.required} required</Badge>}
        />
        <div className="grid grid-cols-2 gap-3 px-5 py-4 md:grid-cols-4 lg:grid-cols-8">
          <Stat label="Pre-arrival" value={m.ssnApplications.blockedBeforeArrival} />
          <Stat label="Ready" value={m.ssnApplications.readyToApply} tone={m.ssnApplications.readyToApply ? 'warn' : 'success'} />
          <Stat label="Online start" value={m.ssnApplications.onlineStarted} tone={m.ssnApplications.onlineStarted ? 'warn' : 'success'} />
          <Stat label="Visit set" value={m.ssnApplications.visitScheduled} tone="progress" />
          <Stat label="Visit done" value={m.ssnApplications.visitCompleted} tone={m.ssnApplications.visitCompleted ? 'warn' : 'success'} />
          <Stat label="Received" value={m.ssnApplications.cardReceived} tone="success" />
          <Stat label="Overdue" value={m.ssnApplications.overdueOfficeVisit} tone={m.ssnApplications.overdueOfficeVisit ? 'danger' : 'success'} />
          <Stat label="Total" value={m.ssnApplications.required} />
        </div>
        <div className="overflow-x-auto border-t border-slate-100 px-5 py-3">
          <table className="w-full text-xs">
            <thead>
              <tr className="text-left text-[11px] uppercase tracking-wide text-slate-400">
                ...
                <th className="pb-2 pr-3 font-medium">Status</th>
                <th className="pb-2 pr-3 font-medium">SSA deadline</th>
                <th className="pb-2 font-medium">Next Navigator action</th>
              </tr>
            </thead>
            <tbody>
              {m.ssnApplications.rows.slice(0, 12).map((r) => (
                <tr key={r.candidateId} className="border-t border-slate-100">
                  <td className="py-2 pr-3 font-medium text-slate-700">{r.candidateName}</td>
                  <td className="py-2 pr-3 text-slate-500">{r.state ?? '—'}</td>
                  <td className="py-2 pr-3"><Badge tone={r.risk === 'red' ? 'danger' : r.risk === 'orange' ? 'warn' : r.risk === 'green' ? 'success' : 'neutral'}>{ssnStatusLabel[r.status] ?? r.status}</Badge></td>
                  <td className="py-2 pr-3 text-slate-500">{r.dueDate ?? '—'}</td>
                  <td className="py-2 text-slate-600">{r.nextAction}</td>
                </tr>
              ))}
              {m.ssnApplications.rows.length === 0 && (
                <tr><td colSpan={5} className="py-5 text-center text-slate-400">No required-state SSN blockers.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Card>
        <CardHeader
          ...
          subtitle="Profiles → start, across the whole pipeline — the counts the Florence OS model runs on."
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <Card>
          <CardHeader title="Production ledger" subtitle="Recent milestones → Florence OS." />
