...
function Flow({ draft, targetState, workflowId, onClose, onDone }: { draft: FormDraft; targetState?: string; workflowId: string; onClose: () => void; onDone: () => void }) {
  const st = resolveEndorsement(targetState)
        ...
        {st && (
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {english ? <Badge tone={english.routingImpact === 'clear' ? 'success' : 'warn'}>English gate</Badge> : null}
            </div>
            ...
            {english && <p className={`mt-2 rounded px-2 py-1 text-[11px] ${english.routingImpact === 'clear' ? 'bg-emerald-50 text-emerald-700' : english.routingImpact === 'needs_documentation' ? 'bg-sky-50 text-sky-700' : 'bg-amber-50 text-amber-800'}`}><strong>English:</strong> {english.label}. {english.florenceAction}</p>}
          </div>
        )}
