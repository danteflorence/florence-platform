...
export function StatePicker({ candidateId, current, onClose, onDone }: { candidateId: string; current?: string; onClose: () => void; onDone: () => void }) {
  ...
  const english = st?.englishRequirement
  ...
  const englishTone = english?.routingImpact === 'clear'
    ? 'bg-emerald-50 text-emerald-700'
    : english?.routingImpact === 'needs_documentation'
            ...
            {st && (
              <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex flex-wrap justify-end gap-1.5">{st.compact && <Badge tone="info">Compact</Badge>}{st.feeUsd ? <Badge tone="neutral">${st.feeUsd}</Badge> : null}{st.timelineDays ? <Badge tone="neutral">~{st.timelineDays[0]}–{st.timelineDays[1]}d</Badge> : null}{english ? <Badge tone={english.routingImpact === 'clear' ? 'success' : 'warn'}>English</Badge> : null}</div>
                </div>
                ...
                {english && <p className={`mt-1.5 rounded px-2 py-1 text-[11px] ${englishTone}`}><strong>English:</strong> {english.label}. {english.florenceAction}</p>}
