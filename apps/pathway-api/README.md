...
server/                 Express API. Production target is Postgres; the legacy
                        SQLite adapter is local throwaway development only.
  db.ts                 Data layer selector; defaults to Postgres and gates local SQLite
  db.postgres.ts        Production Postgres adapter
  db.sqlite.ts          Legacy local adapter gated by PATHWAY_DB=sqlite + ALLOW_SQLITE_LOCAL_DEV=1
  db/                   Postgres schema + migration entrypoint
...
Requires the local toolchain Node 24. Production runs on networked Postgres and
fails closed without `DATABASE_URL`. Local dev and smoke tests default to in-memory
PGlite so they exercise the Postgres-dialect path without writing DB files.
...
```bash
npm run api          # API only; DATABASE_URL uses networked Postgres, otherwise in-memory PGlite
npm run api:local    # API only in explicit local SQLite mode
npm run web          # Vite only
npm run typecheck    # tsc --noEmit (passes clean)
npm run migrate      # apply the Postgres schema to DATABASE_URL
npm run seed         # seed synthetic demo data into the selected Postgres/PGlite backend
npm run seed:local   # explicit local SQLite seed path
```
