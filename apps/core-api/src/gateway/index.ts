...
export function buildGatewayRoutes(deps: GatewayDeps): GwRoute[] {
  ...
  const healthRoute = compileGw({
    ...
    handler: async () => {
      try {
        await deps.store.listSigningKeys();
        return {
          status: 200,
          body: {
            ok: true,
            service: "florence-core",
            env: process.env.FLORENCE_ENV ?? "local",
            database: {
              backend: config.databaseUrl ? "postgres" : config.stateFile ? "file-local-dev" : "memory-local-dev",
              migration: { ok: true, schema: "core-postgres-schema-v1" },
            },
          },
        };
      } catch {
        return {
          ...
          body: {
            ...
            database: {
              backend: config.databaseUrl ? "postgres" : config.stateFile ? "file-local-dev" : "memory-local-dev",
              migration: { ok: false, schema: "core-postgres-schema-v1" },
            },
          },
        };
      }
