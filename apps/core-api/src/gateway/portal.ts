...
<title>Florence OS Platform API — Developer Portal</title>
<style>
...
<body>
<h1>Florence OS Platform API</h1>
<p class="sub">Headless nurse-production platform from Florence Education. The Nurse Passport is the central object (permissioned views); the Production Ledger is the system of record; every workflow is an event. Auth: Core RS256 — <code>fl_session</code> cookie or <code>Authorization: Bearer</code>.</p>
...
<div class="note">
  <p><b>Base URLs.</b> Sandbox <code>https://sandbox-api.florenceedu.com/v1</code> (seeded fake data — test here) · Production <code>https://api.florenceedu.com/v1</code>.</p>
  <p><b>Auth.</b> Machine-to-machine OAuth2: <code>POST https://auth.florenceedu.com/oauth/token</code> with <code>grant_type=client_credentials</code> + your <code>client_id</code>/<code>client_secret</code>, then send <code>Authorization: Bearer &lt;token&gt;</code>. Tokens are scoped to your role; lender keys are org-bound + consent-gated.</p>
