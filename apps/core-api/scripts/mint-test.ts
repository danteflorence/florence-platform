...
{
  const { token, claims } = mintUserSession(keys, mkUser("usr_ops", "ops@florenceedu.com"), [grant("usr_ops", "ops")]);
...
{
  const { claims } = mintUserSession(keys, mkUser("usr_r", "rep@florenceedu.com"), [grant("usr_r", "rep", undefined, "CA,NV")]);
...
{
  const { claims } = mintUserSession(keys, mkUser("usr_n", "new@florenceedu.com"), []);
...
{
  const { token } = mintUserSession(keys, mkUser("usr_t", "t@florenceedu.com"), [grant("usr_t", "ops")]);
