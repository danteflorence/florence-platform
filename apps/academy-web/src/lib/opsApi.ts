...
import type { LibraryResource } from "./libraryApi";
export type { LibraryResource } from "./libraryApi";
...
/** True when connected - via the shared Florence Core staff cookie OR an M2M token. */
...
/** Connect with the shared Florence Core staff cookie - no pasted credentials.
  ...
  try {
    res = await fetch(`${root}/oauth/token`, {
      ...
      body: new URLSearchParams({
        ...
        scope: "candidates:read enrollment:read performance:read payments:read outcomes:read schools:read cohorts:read leads:read leads:write outreach:read outreach:write library:read library:write",
      }),
    });
  } catch {
...
export async function fetchLibraryResources(): Promise<LibraryResource[]> {
  const { token, base } = opsSession();
  if (!base) throw new OpsError(401, "not connected");
  const res = await fetch(`${base}/v1/academy/library/resources`, {
    credentials: "include",
    headers: token ? { authorization: `Bearer ${token}` } : {},
  });
  if (res.status === 401) throw new OpsError(401, "session expired");
  if (!res.ok) throw new OpsError(res.status, "failed to load library resources");
  const j = (await res.json()) as { data: LibraryResource[] };
  return j.data ?? [];
}

export async function uploadLibraryPdf(input: {
  title: string;
  description?: string;
  tags?: string[];
  visibility: "academy" | "cohort";
  cohort?: string;
  file: File;
}): Promise<LibraryResource> {
  const { token, base } = opsSession();
  if (!base) throw new OpsError(401, "not connected");
  if (input.file.type !== "application/pdf" && !/\.pdf$/i.test(input.file.name)) {
    throw new OpsError(400, "Only PDF files can be uploaded here.");
  }
  const headers: Record<string, string> = { "content-type": "application/json" };
  if (token) headers.authorization = `Bearer ${token}`;
  const res = await fetch(`${base}/v1/academy/library/import`, {
    method: "POST",
    credentials: "include",
    headers,
    body: JSON.stringify({
      ...
      cohort: input.visibility === "cohort" ? input.cohort : undefined,
      tags: input.tags,
      file_name: input.file.name,
      mime_type: "application/pdf",
      content_base64: await fileToBase64(input.file),
    }),
  });
  const j = (await res.json().catch(() => null)) as
    | (LibraryResource & { error?: { message?: string } })
    | null;
  if (res.status === 401) throw new OpsError(401, "session expired");
  if (!res.ok) throw new OpsError(res.status, j?.error?.message ?? "failed to upload PDF");
  return j as LibraryResource;
}
...
export type AccessStatus = "paid" | "pending" | "failed" | "none";
...
export function nextBestAction(
  ...
  if (stage === "registered" && !accessActive) return "Follow up on Global Live access";
...
export function buildRoster(data: OpsData): RosterRow[] {
  ...
  const accessByCand = new Map<string, AccessInfo>();
  const rank = (s: AccessStatus) => (s === "paid" ? 3 : s === "pending" ? 2 : s === "failed" ? 1 : 0);
  for (const p of payments) {
    ...
    const cur = accessByCand.get(p.candidate_id);
    if (!cur || rank(status) > rank(cur.status)) {
      accessByCand.set(p.candidate_id, { status, amountCents: p.amount_cents, currency: p.currency });
    }
  }
  ...
  const rows: RosterRow[] = candidates.map((c) => {
    ...
    const access = accessByCand.get(c.id) ?? { status: "none" as const };
    const accessActive = access.status === "paid";
    ...
    return {
      ...
      nextAction: nextBestAction(stage, band, accessActive, topFocusLabel),
      theta: latest?.theta,
