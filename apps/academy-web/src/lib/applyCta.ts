...
export function applySessionId(): string {
  try {
    ...
    if (existing && SAFE_SESSION_RE.test(existing)) return existing;
    const raw =
...
export function buildApplyUrl(input: {
  ...
  url.searchParams.set("session_id", input.sessionId && SAFE_SESSION_RE.test(input.sessionId) ? input.sessionId : applySessionId());
