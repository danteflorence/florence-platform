...
export async function call<T>(
  ...
  try {
    res = await fetch(`${base}${path}`, {
      ...
      credentials: "include", // carry the shared Florence Core cookie (SSO)
      headers,
...
// ── Academy Live residency + Access Grants ──────────────────────────────────
...
export interface AcademyCamp {
  ...
  seat_capacity: 50 | 75 | 100;
  confirmed?: number;
  waitlisted?: number;
  seats_remaining?: number;
}

export interface AcademyResidency {
  id: string;
...
export interface GapCalculationResponse {
  gap: {
    id: string;
    candidate_id: string;
    ...
    status: "gap_identified" | "documents_missing" | "gap_verified";
  };
  eligibility: {
    eligible: boolean;
    eligible_amount_usd: number;
...
export async function fetchResidencies(): Promise<AcademyResidency[]> {
  const base = apiBaseUrl();
  if (!base) return [];
  try {
    const res = await fetch(`${base}/v1/academy/residencies`);
    if (!res.ok) return [];
    const json = (await res.json()) as { data?: AcademyResidency[] };
    return json.data ?? [];
  } catch {
    return [];
  }
...
export async function requestResidencySeat(campId: string): Promise<CampReservation> {
  return call<CampReservation>("/v1/academy/residency/reservations", {
    method: "POST",
    body: { camp_id: campId },
  });
}

export async function createGrantGap(input: {
  pathway_stage: string;
  estimated_gap_usd: number;
  verified_gap_usd?: number;
  status?: "gap_identified" | "documents_missing" | "gap_verified";
  documents_required?: string[];
}): Promise<GapCalculationResponse> {
  return call<GapCalculationResponse>("/v1/academy/grants/gaps", {
    method: "POST",
    body: input,
  });
}

export async function fetchMyAccessGrants(): Promise<AccessGrant[]> {
  const res = await call<{ data: AccessGrant[] }>("/v1/academy/grants");
  return res.data ?? [];
}
