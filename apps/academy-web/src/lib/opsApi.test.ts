...
describe("buildRoster", () => {
  ...
  it("derives stage, band, sponsored access, and latest readiness per candidate", () => {
    const a = byName("A");
    ...
    expect(a.accessActive).toBe(true);
    expect(a.access.status).toBe("paid");
    expect(a.access.amountCents).toBe(10000);
    expect(a.assessmentsCount).toBe(2);

    const c = byName("C");
    expect(c.band).toBe("none"); // unassessed
    expect(c.accessActive).toBe(false);
    expect(c.access.status).toBe("none"); // c3's only payment is tuition, not Global Live access
  });
