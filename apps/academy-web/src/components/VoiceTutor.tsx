...
export default function VoiceTutor() {
  ...
  useEffect(() => {
    fetchTutorConfigured().then((c) => {
      setConfigured(c);
      setTutorConfigured(c); // lets "Ask Florence about this" buttons appear
    });
  }, []);
...
function TutorWidget() {
  ...
  // A walkthrough's "Ask Florence about this" opens the panel pre-seeded.
      ...
      {open && (
        <div className="fixed bottom-20 right-5 z-40 w-80 rounded-2xl border border-florence-line bg-white p-4 shadow-2xl">
          <div className="flex items-center justify-between">
            <p className="text-sm font-bold text-florence-ink">Florence</p>
          ...
          <div className="mt-3 flex items-center gap-2 text-xs">
            ...
            <span className="text-florence-slate">
              {active ? (isSpeaking ? "Florence is speaking…" : "Listening…") : connecting ? "Connecting…" : "Not connected"}
            </span>
          </div>
