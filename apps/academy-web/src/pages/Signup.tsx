...
function SidebarBrief({
  ...
  return (
    <aside>
      ...
      <FlorenceCard className="mt-8 space-y-4 p-6">
        <Row label="Cohort">
        ...
        <Row label="Apply">
          <span className="font-semibold text-florence-ink">Fee covered</span>
          <span className="block text-xs text-florence-slate">
            Florence tracks partner-program application fee coverage.
          </span>
        </Row>
      </FlorenceCard>
...
function SignupForm({
  ...
  if (phase === "redirecting") {
    return (
      <FlorenceCard className="p-10 text-center">
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-2xl border border-florence-line bg-white p-8">
      ...
      <div className="mt-6 space-y-4">
        ...
        <FormField label="Country" hint="Optional. Used to surface the right cohort.">
          <input
            type="text"
      ...
      {error && (
        <AlertBanner tone="danger" title={error} className="mt-4" />
      )}
