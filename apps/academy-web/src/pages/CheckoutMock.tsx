  ...
  return (
    <div className="grid min-h-screen place-items-center bg-florence-ink px-5">
      <div className="w-full max-w-md rounded-2xl border border-florence-line bg-white p-6 shadow-card-lg">
        ...
        <div className="mt-5 divide-y divide-florence-line rounded-lg border border-florence-line bg-white">
          <CheckoutRow label="Program value" value="$200" />
          <CheckoutRow label="University sponsorship" value="-$100" />
          <CheckoutRow label="Student pays" value={amountLabel} strong />
          <CheckoutRow label="Application fee" value="Covered by Florence" />
        </div>
...
function CheckoutRow({
  ...
  return (
    <div className="flex items-baseline justify-between gap-4 px-4 py-3">
      <span className="text-sm font-medium text-florence-slate">{label}</span>
      <span className={strong ? "text-2xl font-semibold tabular-nums text-florence-ink" : "text-sm font-semibold text-florence-ink"}>
        {value}
      </span>
    </div>
  );
