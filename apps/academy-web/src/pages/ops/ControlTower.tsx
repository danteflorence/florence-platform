...
type Tab = "dashboard" | "library" | "leads" | "outreach";
...
export default function ControlTower() {
  ...
  useEffect(() => {
    ...
    // Try the shared Florence Core staff cookie (SSO) before asking to connect.
        ...
        {status === "ready" && metrics && (
          <>
            ...
            <nav className="mt-6 flex gap-1 border-b border-white/10">
              {(["dashboard", "library", "leads", "outreach"] as Tab[]).map((t) => (
                <button
                  key={t}
                  type="button"
                  onClick={() => setTab(t)}
...
function LibraryAdminPanel() {
  const [resources, setResources] = useState<LibraryResource[]>([]);
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [tags, setTags] = useState("");
  const [visibility, setVisibility] = useState<"academy" | "cohort">("academy");
  ...
  const loadResources = useCallback(async () => {
    try {
      setResources(await fetchLibraryResources());
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not load library resources.");
    }
  }, []);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (visibility === "cohort" && !cohort.trim()) {
      setError("Enter a cohort code for a class-scoped PDF.");
      return;
    }
    setStatus("Uploading PDF...");
    try {
      await uploadLibraryPdf({
        title: title.trim() || file.name,
        description: description.trim() || undefined,
        tags: tagsFromInput(tags),
        visibility,
        cohort: visibility === "cohort" ? cohort.trim() : undefined,
        file,
      });
      setTitle("");
      setDescription("");
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
      <Panel title="Upload PDF" subtitle="Admin-only study library publishing">
        <form className="space-y-3" onSubmit={submit}>
          <label className="block">
            ...
            <input
              ...
              className="mt-1 w-full rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-florence-teal"
              placeholder="NCLEX delegation handout"
            />
          </label>
          <label className="block">
            ...
            <textarea
              ...
              className="mt-1 min-h-24 w-full resize-y rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-florence-teal"
              placeholder="What this PDF is for"
            />
          </label>
          <label className="block">
            <span className="text-xs font-semibold uppercase tracking-wide text-white/50">Tags</span>
            <input
              value={tags}
              onChange={(event) => setTags(event.target.value)}
              className="mt-1 w-full rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-florence-teal"
              placeholder="prioritization, delegation, pharmacology"
            />
          </label>
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wide text-white/50">Visibility</span>
              <select
                value={visibility}
                onChange={(event) => setVisibility(event.target.value as "academy" | "cohort")}
                className="mt-1 w-full rounded-lg border border-white/10 bg-florence-ink px-3 py-2 text-sm text-white outline-none focus:border-florence-teal"
              >
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wide text-white/50">Cohort code</span>
              <input
                value={cohort}
                onChange={(event) => setCohort(event.target.value)}
                disabled={visibility !== "cohort"}
                className="mt-1 w-full rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-florence-teal disabled:opacity-40"
                placeholder="MNL-2026-07"
              />
            </label>
          </div>
          <label className="block rounded-xl border border-dashed border-white/15 bg-white/[0.03] p-4">
            <span className="text-xs font-semibold uppercase tracking-wide text-white/50">PDF file</span>
            <input
              type="file"
            ...
            />
            <span className="mt-2 block text-xs text-white/40">PDF only. Current upload limit is 750 KB.</span>
          </label>
          <button
            type="submit"
            className="w-full rounded-xl bg-florence-teal px-5 py-2.5 text-sm font-semibold text-florence-ink hover:bg-florence-teal/90"
          >
            Publish PDF
          </button>
          {(status || error) && (
            <p className={`rounded-lg px-3 py-2 text-sm ${error ? "bg-vital-danger/15 text-vital-danger" : "bg-florence-teal/15 text-florence-teal"}`}>
              {error || status}
            </p>
          )}
        </form>
      ...
      <Panel title="Published Library" subtitle="Resources visible to students by permission">
        <div className="space-y-2">
          {resources.map((resource) => (
            <div key={resource.id} className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <p className="font-semibold text-white">{resource.title}</p>
                  <p className="mt-0.5 text-xs text-white/45">
                    {resource.document?.file_name ?? resource.embed?.origin ?? resource.kind}
                  </p>
                </div>
                <span className="rounded-full bg-white/10 px-2 py-1 text-xs font-semibold text-white/60">
                  {resource.visibility === "cohort" ? resource.cohort ?? "cohort" : resource.visibility}
                </span>
              </div>
              {resource.description && <p className="mt-2 text-sm leading-6 text-white/60">{resource.description}</p>}
            </div>
          ))}
          {resources.length === 0 && (
            <p className="rounded-lg bg-white/[0.03] px-3 py-2 text-sm text-white/45">
              No PDFs or interactive resources have been published yet.
            </p>
          )}
        </div>
      </Panel>
...
function CandidatesPanel({ roster }: { roster: RosterRow[] }) {
  ...
  const isFollowUp = (r: RosterRow) => r.access.status !== "paid" && r.stage !== "withdrawn";
  ...
  const filtered = useMemo(() => {
    ...
    return roster.filter((r) => {
      ...
      if (access === "paid" && r.access.status !== "paid") return false;
      if (access === "pending" && r.access.status !== "pending") return false;
      if (access === "unpaid" && r.access.status === "paid") return false;
          ...
          <tbody className="text-white/85">
            {filtered.map((r) => (
              ...
              >
                ...
                <td className="py-2.5 pr-4">
                  {r.access.status === "paid" ? (
                    <span className="font-medium text-vital-ok">{accessAmount(r.access)}</span>
                  ) : r.access.status === "pending" ? (
                    <span className="text-amber-400">Pending</span>
                  ) : r.access.status === "failed" ? (
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          ...
          <DetailStat
            ...
            value={
              row.access.status === "paid"
                ? `${accessAmount(row.access)} active`
                : row.access.status === "pending"
                  ? "Pending"
                  : row.access.status === "failed"
                    ? "Failed"
                    : "Not started"
            }
            dot={ACCESS_DOT[row.access.status]}
          />
        </div>
    ...
    <div className="mx-auto grid min-h-[80vh] max-w-md place-items-center px-5">
      <form onSubmit={onSubmit} className="w-full rounded-2xl border border-white/10 bg-white/[0.04] p-6">
        ...
        <p className="mt-1 text-sm text-white/50">
          Operator access. Sign in with your Florence account (one login across the platform),
