...
// Florence Core SSO cookie name (shared across the fleet).
...
/**
 * Authenticate a request. Now async: it first tries the Florence Core RS256
...
export async function authenticate(authHeader?: string, cookieHeader?: string): Promise<AuthResult> {
  // 1) Florence Core SSO (RS256 via JWKS) - bearer or shared cookie.
