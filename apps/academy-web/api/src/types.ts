...
export type SponsorProgram = SponsorshipProgram;
...
export type SponsoredAccessPass = AccessPass;
...
// can see sponsored Global Live access, but Florence makes NO endorsement claim
...
// ── Live Residency Manager + Florence Access Grants ────────────────────────
export type ResidencyLocation = "manila" | "los_angeles";
export type ResidencyStatus = "scheduled" | "active" | "completed" | "cancelled";
...
export type CampOption = "morning" | "evening";
export type CampStatus = "scheduled" | "active" | "completed" | "cancelled";
export type CampSeatCapacity = 50 | 75 | 100;
...
export type CampReservationStatus = "requested" | "confirmed" | "waitlisted" | "cancelled";
...
export type GapCalculationStatus = "gap_identified" | "documents_missing" | "gap_verified";
...
export interface GrantApprovalQueue {
  id: string;
  access_grant_id: string;
  status: "eligible_for_review" | "approved" | "declined";
  reviewer?: string;
  queued_at: string;
  decided_at?: string;
}
...
export interface Disbursement {
  ...
  access_grant_id: string;
  amount_usd: number;
  recipient_type: "school" | "vendor";
  recipient_name: string;
  status: "disbursement_pending" | "disbursed" | "cancelled_refunded";
  direct_payment: true;
  created_at: string;
  updated_at: string;
  disbursed_at?: string;
...
export interface Receipt {
  ...
  status: "received" | "receipt_qa_approved" | "rejected";
  receipt_ref?: string;
  created_at: string;
  updated_at: string;
  qa_at?: string;
 ...
 *   reserved   - started sponsored access checkout
 *   enrolled   - activated sponsored access / became a candidate
