...
try {
  ...
  const fake2: SqlClient = {
    query: async (sql: string, params: unknown[] = []): Promise<any[]> => {
      ...
      if (/^INSERT INTO academy_library_resources/i.test(sql)) {
        libStore.push({
          id: params[0],
          kind: params[1],
          title: params[2],
          ...
          cohort: params[6],
          tags: params[7],
          document: JSON.parse(String(params[8])),
          embed: params[9],
          content_enc: params[10],
      ...
      if (/^SELECT \* FROM academy_library_resources WHERE id/i.test(sql)) {
        return libStore.filter((r) => r["id"] === params[0]);
      }
      if (/^SELECT content_enc FROM academy_library_resources WHERE id/i.test(sql)) {
        return libStore.filter((r) => r["id"] === params[0]).map((r) => ({ content_enc: r["content_enc"] }));
      }
      if (/^SELECT \* FROM academy_library_resources/i.test(sql)) return libStore;
      return [];
    },
  };
  ...
  const bodyBase64 = Buffer.from("safe study note", "utf8").toString("base64");
  const lib = await pg2.library.create({
    kind: "document",
    title: "Study note",
    visibility: "private",
    candidate_id: "cand_x",
    tags: ["safe"],
    document: {
      file_name: "study-note.txt",
      mime_type: "text/plain",
      size_bytes: 15,
      content_sha256: "a".repeat(64),
    },
    content_base64: bodyBase64,
    created_by: "candidate:cand_x",
  });
  const libInsert = libStore.find((r) => r["id"] === lib.id);
  assert.ok(libInsert);
  assert.notEqual(libInsert?.["content_enc"], bodyBase64);
  assert.equal((await pg2.library.getDocumentContent(lib.id))?.content_base64, bodyBase64);
  ok("PostgresStore library document body is encrypted and decrypts through signed-access path");
