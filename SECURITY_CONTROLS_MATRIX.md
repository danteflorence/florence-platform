...
| Webhooks | Inbound webhook verification | Partial, Employer Connect ATS route hardened | `apps/employer-connect-api/server/routes.ts`, Academy webhook code | `webhook-signature-smoke`, Academy API tests | Extend timestamped HMAC, replay windows, idempotency, and rate limits to every remaining inbound provider webhook. |
