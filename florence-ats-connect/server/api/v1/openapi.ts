...
export const OPENAPI_V1: Record<string, unknown> = {
  ...
  paths: {
    ...
    '/applications/eligibility-check': { post: { summary: 'Run the Application Submission Gate', 'x-scope': 'applications:eligibility', responses: { '200': { description: 'gate status + missing[] + allowedAction + subjectTo' } } } },
    '/applications/{packetId}/submit': { post: { summary: 'Submit a packet (hard-gated; audited super_admin override)', 'x-scope': 'applications:submit', parameters: [{ $ref: '#/components/parameters/IdempotencyKey' }], responses: { '201': { description: 'submitted' }, '409': { description: 'gate not cleared (missing[])' } } } },
