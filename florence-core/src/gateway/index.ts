...
export function buildGatewayRoutes(deps: GatewayDeps): GwRoute[] {
  const modules: GwRoute[] = [...nursesModule(deps.store, deps.audit), ...ledgerModule(deps.store, deps.audit), ...modelGatewayModule(deps.audit), ...webhooksModule(deps.store, deps.audit), ...partnerKeysModule(deps.store, deps.audit), ...lenderModule(deps.store, deps.audit)];
...
export function createGateway(deps: GatewayDeps, opts: GatewayOpts = {}): (ctx: import("../server.ts").Ctx) => Promise<boolean> {
  return createGatewayDispatch(buildGatewayRoutes(deps), deps.keys, deps.store, opts);
}
